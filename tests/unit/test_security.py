...
def test_s2_execution_trap(auditor):
    ...
    assert any("Insecure Output Handling" in f.title for f in findings)
...
def test_s6_tool_overprivilege(auditor):
    ...
    assert any("Tool Over-Privilege" in f.title for f in findings)
...
def test_s7_kb_poisoning(auditor):
    ...
    assert any("Integrity Poisoning" in f.title for f in findings)
