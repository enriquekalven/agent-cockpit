...
class CockpitLlamaIndexCallbackHandler(BaseCallbackHandler):
    ...
    def on_event_start(
        ...
        **kwargs: Any,
        ...
        if event_type == CBEventType.LLM:
            ...
            for prompt in prompts:
                ...
                if scrubbed != str(prompt):
                    logger.warning(
                        "[Cockpit] PII detected in LlamaIndex LLM call"
                    )
        ...
        if event_type == CBEventType.FUNCTION_CALL:
            tool_name = payload.get(EventPayload.TOOL, {}).get(
                "name", "unknown_tool"
            )
            ...
            if any(
                x in tool_name.lower()
                for x in ["mcp", "shell", "exec", "system"]
            ):
                self.findings.append(
                    {
                        "category": "🛡️ Security",
    ...
    def end_trace(
        ...
        if self.findings:
            logger.info(
                f"[Cockpit] Audit complete for {self.app_name}. Syncing {len(self.findings)} snippets to Evidence Lake."
            )
