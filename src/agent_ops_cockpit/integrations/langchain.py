...
class CockpitCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(
        self, app_name: str = "langchain-agent", project_path: str = "."
    ):
    ...
    def on_llm_start(
        ...
        for prompt in prompts:
            ...
            if scrubbed != prompt:
                logger.warning(
                    f"[Cockpit] PII detected in prompt for {self.app_name}"
                )
    ...
    def on_tool_start(
        ...
        if any(
            ...
            for x in ["delete", "remove", "drop", "wipe", "terminate"]
        ):
    ...
    def on_chain_end(self, outputs: Dict[str, Any], **kwargs: Any) -> Any:
        ...
        if self.findings:
            ...
            logger.info(
                f"[Cockpit] Audit complete for {self.app_name}. {len(self.findings)} findings generated."
            )
                    ...
                    with open(lake_path, "r") as f:
                        try:
                            ...
                            if isinstance(content, list):
                                ...
                                logger.info(
                                    "[Cockpit] Evidence lake is a dict, starting fresh list."
                                )
                                data = [content]
