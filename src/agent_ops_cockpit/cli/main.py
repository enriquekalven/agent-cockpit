...
app = typer.Typer(
    help="🕹️ AgentOps Cockpit: The Cockpit Fleet Governance Platform.",
    no_args_is_help=False,
)
audit_app = typer.Typer(
    help="🛡️ Audit Hub: Verify security, quality, arch, and compliance."
)

# --- MASTER HUB ---
cockpit_app = typer.Typer(
    help="🚀 Cockpit: Manage your agent fleet and project health."
)
...
def cockpit_main(
    ctx: typer.Context,
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to workspace")
    ] = ".",
):
...
def bootstrap(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to project")
    ] = ".",
):
    """🏗️ Bootstrap: Explicitly initialize Cockpit manifests and adopt cockpit libraries."""
    console.print(
        Panel.fit(
            "🏗️ [bold blue]COCKPIT BOOTSTRAP: PROJECT INITIALIZATION[/bold blue]",
            border_style="blue",
        )
    )
    ...
    console.print(
        "✅ Generated [bold]Cockpit Scaffolding[/bold] (policy_engine.ts, etc.)"
    )
...
fleet_app = typer.Typer(
    help="🛰️ Fleet Hub: Day 2 Ops, Health Tracking, and FinOps scaling."
)
deploy_app = typer.Typer(
    help="🚀 Deployment Hub: Project hydration, migration, and cloud factory."
)
fix_app = typer.Typer(
    help="🔧 Evolution Hub: Targeted fixes and autonomous code synthesis."
)
test_app = typer.Typer(
    help="🧪 Reliability Hub: Unit tests and Persona smoke tests."
)
sys_app = typer.Typer(
    help="🩺 System Hub: Health diagnosis and version tracking."
)
ops_app = typer.Typer(
    help="🛡️ Operations Hub: Observability bridges, Shadow Routing, and Runtime Watchers."
)
create_app = typer.Typer(
    help="🏗️ Scaffolding Hub: Project initialization and UI creation."
)
context_app = typer.Typer(
    help="🧠 Context Hub: Manage agent knowledge, skills, and annotations."
)
release_app = typer.Typer(
    help="🚀 Release Hub: Autonomous versioning, changelogs, and global deployment."
)
...
def legacy_report(
    ...
    workspace: bool = typer.Option(
        ...
        "--workspace",
        "-w",
        help="Scan and audit all agents in the workspace",
    ),
    heal: bool = typer.Option(
        False, "--heal", help="Deprecated: Use --apply-fixes instead"
    ),
):
    """[DEPRECATED] Master Architect: Auto-aliasing to 'audit report'."""
    console.print(
        "🔄 [bold yellow]Master Architect:[/] Auto-aliasing legacy 'report' to [bold blue]'audit report'[/bold blue]..."
    )
    if heal:
        console.print(
            "🔧 [dim]Detected legacy --heal flag. Mapping to --apply-fixes...[/dim]"
        )
...
def callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        ...
        telemetry.track_event_sync(
            "cli_command", {"command": ctx.invoked_subcommand}
        )
...
def diagnose():
    ...
    console.print(
        Panel.fit(
            "🩺 [bold blue]AGENTOPS COCKPIT: SYSTEM DIAGNOSIS (DOCTOR)[/bold blue]",
            border_style="blue",
        )
    )
    ...
    try:
        ...
        table.add_row(
            "GCP Auth (ADC)",
            f"[green]PASSED ({project})[/green]",
            "Authenticated",
        )
    except Exception:
        table.add_row(
            "GCP Auth (ADC)",
            "[red]REAUTHENTICATION NEEDED[/red]",
            "Run 'gcloud auth application-default login'",
        )
    if has_cockpit:
        table.add_row(
            "Artifact Store",
            "[green].cockpit/ (Detected)[/green]",
            "Cockpit data path OK",
        )
    else:
        table.add_row(
            "Artifact Store",
            "[yellow]NOT INITIALIZED[/yellow]",
            "Run 'agent-ops audit report' to bootstrap",
        )
    try:
        import urllib.request

        with urllib.request.urlopen(
            config.PUBLIC_PYPI_URL, timeout=2
        ) as response:
        ...
        table.add_row(
            "Registry Access",
            "[yellow]OFFLINE / RESTRICTED[/yellow]",
            "Check VPN or use --public",
        )
    if os.path.exists("src/agent_ops_cockpit/agent.py") or os.path.exists(
        "src/a2ui"
    ):
        table.add_row(
            "Trinity Structure",
            "[green]VERIFIED[/green]",
            "Engine/Face folders present",
        )
    else:
        table.add_row(
            "Trinity Structure",
            "[yellow]NON-STANDARD[/yellow]",
            "Running from external agent home",
        )
    console.print(table)
    console.print(
...
def upgrade_cockpit():
    ...
    console.print(
        Panel.fit(
            "🚀 [bold blue]AGENTOPS COCKPIT: COCKPIT UPGRADE ENGINE[/bold blue]",
            border_style="blue",
        )
    )
    console.print(
        f"Current Local Version: [bold cyan]v{config.VERSION}[/bold cyan]"
    )
    ...
    try:
        ...
        with urllib.request.urlopen(
            "https://pypi.org/pypi/agentops-cockpit/json", timeout=3
        ) as response:
            if response.status == 200:
                data = json.load(response)
                latest_version = data["info"]["version"]
                console.print(
                    f"Latest PyPI Version:  [bold green]v{latest_version}[/bold green]"
                )
                ...
                if latest_version != config.VERSION:
                    ...
                    console.print(
                        Panel(
                            f"[bold green]pip install --upgrade agentops-cockpit --index-url {config.PUBLIC_PYPI_URL}[/bold green]",
                            border_style="cyan",
                        )
                    )
                else:
        ...
        console.print(
            Panel(
                "[bold green]pip install --upgrade agentops-cockpit[/bold green]",
                border_style="dim",
            )
        )
...
def list_models():
    ...
    console.print(
        Panel.fit(
            "🧠 [bold blue]AGENTOPS COCKPIT: MODEL CAPABILITY DISCOVERY[/bold blue]",
            border_style="blue",
        )
    )
    try:
        ...
        for m in models:
            ...
            if "gemini" in m.name.lower():
                methods = ", ".join(
                    [
                        meth.split(":")[0]
                        for meth in m.supported_generation_methods
                    ]
                )
        ...
        console.print(
            "[dim]Hint: Ensure GOOGLE_API_KEY is set or run 'gcloud auth application-default login'.[/dim]"
        )
...
def certification(
    path: Annotated[
        str,
        typer.Option(
            "--path", "-p", help="Path to the agent project to certify"
        ),
    ] = ".",
    no_interactive: Annotated[
        bool,
        typer.Option("--no-interactive", help="Run in non-interactive mode"),
    ] = False,
    ...
    console.print(
        Panel.fit(
            "🏅 [bold blue]AGENTOPS COCKPIT: PRODUCTION READINESS CERTIFICATION[/bold blue]",
            border_style="blue",
        )
    )
    ...
    audit_exit_code = orch_mod.run_audit(
        mode="deep",
        target_path=path,
        title="PRODUCTION CERTIFICATION AUDIT",
        apply_fixes=True,
    ...
    if audit_exit_code == 0 and regression_passed:
        console.print(
            Panel.fit(
                "🏆 [bold green]CERTIFICATION GRANTED[/bold green]\nAgent is ready for Production Deployment to Google Cloud / AWS.",
                border_style="green",
            )
        )
        ...
        console.print(
            f"📜 [bold cyan]Cockpit Certificate Generated:[/bold cyan] {cert_path}"
        )
        ...
        secret = os.environ.get(
            "COCKPIT_COCKPIT_SECRET", "super-secret-cockpit-key-v202"
        )
        payload = {
        ...
        console.print(
            f"🛡️  [bold green]Cockpit Identity Issued (MuTI):[/bold green] {token_path}"
        )
        console.print(
            "🧩 [dim]This JWT enables Mutual-TLS-for-Intelligence handshakes.[/dim]"
        )
    else:
        console.print(
            Panel.fit(
                "🛑 [bold red]CERTIFICATION DENIED[/bold red]\nCritical gaps detected in Security, Reliability or Logic.",
                border_style="red",
            )
        )
        console.print(
            "[dim]Review the coaching reports above and in the .cockpit/ directory for remediation steps.[/dim]"
        )
...
def report(
    ...
    path: Annotated[
        str,
        typer.Option(
            "--path", "-p", help="Path to the agent or workspace to audit"
        ),
    ] = ".",
    workspace: Annotated[
        bool,
    ...
    apply_fixes: Annotated[
        ...
        typer.Option(
            ...
            "-f",
            "--heal",
            help="Automatically apply recommended fixes (Auto-Remediation)",
        ),
    ] = False,
    sim: Annotated[
        bool,
        typer.Option(
            "--sim", help="Run in simulation mode (Synthetic SME reasoning)"
        ),
    ] = False,
    public: Annotated[
        bool,
        typer.Option(
            "--public",
            help="Force use of public PyPI for registry checks (handles 401 errors)",
        ),
    ] = False,
    ...
    plain: Annotated[
        bool,
        typer.Option(
            "--plain", help="Use plain output without complex Unicode boxes"
        ),
    ] = False,
    dry_run: Annotated[
        bool,
        typer.Option(
            "--dry-run",
            help="Simulate fixes without applying them (Dry Run Dashboard)",
        ),
    ] = False,
    interactive: Annotated[
        bool,
        typer.Option(
            "--interactive",
            "-i",
            help="Ask for confirmation before applying each fix",
        ),
    ] = False,
    only: Annotated[
        Optional[List[str]],
        typer.Option(
            "--only",
            help="Only run specific categories (e.g. security, finops)",
        ),
    ] = None,
    skip: Annotated[
        Optional[List[str]],
        typer.Option("--skip", help="Skip specific categories"),
    ] = None,
    verbose: Annotated[
        bool,
        typer.Option(
            "--verbose", "-v", help="Enable verbose output for debugging"
        ),
    ] = False,
):
    ...
    if public:
        ...
        console.print(
            "🌐 [bold cyan]Switching to Public Registry Failover (PyPI)[/bold cyan]"
        )
    if workspace:
        console.print(
            f"🕹️ [bold blue]Launching {mode.upper()} WORKSPACE Audit (v{config.VERSION})...[/bold blue]"
        )
        success = orch_mod.workspace_audit(
            root_path=path,
            mode=mode,
            sim=sim,
        ...
        console.print(
            f"🕹️ [bold blue]Launching {mode.upper()} System Audit (v{config.VERSION})...[/bold blue]"
        )
...
def security(
    ...
    console.print(
        "🚩 [bold red]Launching Security Audit (Red Team + Secrets)...[/bold red]"
    )
...
def document(
    ...
    console.print(
        f"📄 [bold green]Technical Design Document (HTML):[/bold green] {h_out}"
    )
    console.print(
        f"📝 [bold green]Technical Design Document (MD):  [/bold green] {m_out}"
    )
    console.print(
        f"📦 [bold green]Cockpit Codebase Bundle:      [/bold green] {b_out}"
    )
...
def policy(
    input_text: Annotated[
        Optional[str],
        typer.Option("--text", "-t", help="Input text to validate"),
    ] = None,
):
    """Audit declarative guardrails (Forbidden topics, HITL)."""
    console.print(
        "🛡️ [bold green]Launching Guardrail Policy Audit...[/bold green]"
    )
    ...
    if input_text:
        try:
            ...
            console.print(
                "✅ [bold green]Input Passed Validation.[/bold green]"
            )
        except policy_mod.PolicyViolation as e:
            console.print(
                f"❌ [bold red]Violation Detected:[/bold red] {e.category} - {e.message}"
            )
    else:
        report = engine.get_audit_report()
        console.print(
            f"📋 [bold cyan]Policy Engine Status:[/bold cyan] {report['policy_active']}"
        )
        console.print(
            f"🚫 [bold]Forbidden Topics:[/bold] {report['forbidden_topics_count']}"
        )
        console.print(
            f"🤝 [bold]HITL Tools:[/bold] {', '.join(report['hitl_tools'])}"
        )
...
@audit_app.command()
def email(
    recipient: Annotated[str, typer.Argument(help="Recipient email address")],
):
    """Email the latest audit report."""
    console.print(
        f"📡 [bold blue]Preparing to email audit report to {recipient}...[/bold blue]"
    )
    orchestrator = orch_mod.CockpitOrchestrator()
    if not os.path.exists("cockpit_final_report.md"):
        console.print(
            "[red]❌ Error: Run 'agent-ops audit report' first.[/red]"
        )
        return
...
def face(
    path: Annotated[
        str, typer.Option("--path", help="Path to Frontend folder")
    ] = "src",
):
...
def export(target: str = "local", format: str = "json"):
    ...
    console.print(
        f"📡 [bold blue]AgentOps Export: Exporting traces to {target} ({format})...[/bold blue]"
    )
    res = telemetry.export_traces(format=format, target_hub=target)
    if res["status"] == "success":
        console.print(
            f"✅ [bold green]Export Complete![/bold green] {res.get('file', res.get('message'))}"
        )
    else:
        console.print(
            f"❌ [bold red]Export Failed:[/bold red] {res['message']}"
        )
...
def shadow_routing(diversion: float = 0.05):
    ...
    console.print(
        "✨ [bold]Shadow Router in standby.[/bold] Listening for diversion signals..."
    )
...
def gateway(port: int = 8000):
    ...
    console.print(
        Panel.fit(
            "🛡️ [bold blue]COCKPIT GATEWAY: LOCAL SIDECAR[/bold blue]\nListening for agent completions on localhost:8000...",
            border_style="blue",
        )
    )
...
@audit_app.command()
def shadow_analysis(
    base: Annotated[str, typer.Argument(help="Path to base agent/report")],
    candidate: Annotated[
        str, typer.Argument(help="Path to candidate agent/report")
    ],
):
...
def mothball(
    cloud: Annotated[
        Optional[str],
        typer.Option("--cloud", help="Specific cloud to mothball"),
    ] = None,
):
...
def resume(
    cloud: Annotated[
        Optional[str], typer.Option("--cloud", help="Specific cloud to resume")
    ] = None,
):
...
def tunnel(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to local agent")
    ] = ".",
    port: Annotated[int, typer.Option("--port", help="Local port")] = 8080,
):
    """Mocks a production registration for a local agent (GE Bridge)."""
    console.print(
        f"🌉 [bold magenta]Establishing Local-to-Cloud Bridge for port {port}...[/bold magenta]"
    )
    orchestrator = cockpit_mod.CockpitOrchestrator()
    ...
    orchestrator.fleet_manager.register_agent(
        name=agent_name,
        path=path,
        cloud="local-tunnel",
        endpoint=tunnel_url,
    ...
    console.print(
        f"✅ [green]Tunnel Active:[/] Gemini Enterprise now sees [bold]{agent_name}[/] at {tunnel_url}"
    )


@fleet_app.command(name="anomaly")
def anomaly_check(
    name: Annotated[str, typer.Option(help="Agent name to audit")],
    sim_rogue: Annotated[
        bool, typer.Option("--rogue", help="Simulate rogue PII exfiltration")
    ] = False,
):
        ...
        telemetry_data.append(
            {
                "timestamp": datetime.now().isoformat(),
                "tool_calls": 1,
                "payload": "Extracting PII: user@example.com",
            }
        )
    orchestrator = cockpit_mod.CockpitOrchestrator()
    report = orchestrator.fleet_manager.monitor_agent_anomaly(
        name, telemetry_data
    )
...
def fleet_shadow_roi(
    path: Annotated[
        str,
        typer.Option(
            "--path", "-p", help="Path to look for agents to benchmark"
        ),
    ] = ".",
):
...
def cockpit(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to the agent/workspace")
    ] = ".",
    fleet: Annotated[
        bool,
        typer.Option("--fleet", help="Process all agents in the workspace"),
    ] = True,
    target: Annotated[
        str,
        typer.Option(
            "--target", "-t", help="Target Cloud Platform: google, aws, azure"
        ),
    ] = "google",
):
...
def migrate(
    path: Annotated[
        str,
        typer.Option("--path", "-p", help="Path to look for agents to migrate"),
    ] = ".",
    target: Annotated[
        str,
        typer.Option(
    ...
    if not results:
        console.print(
            f"[yellow]No migration candidates found for target: {target}[/yellow]"
        )
    else:
        for r in results:
            reg_info = (
                f" | Registry: [bold magenta]{r['registry']}[/bold magenta]"
                if r["cloud"] == "google"
                else ""
            )
            console.print(
                f"✅ [bold green]Migrated:[/bold green] {r['agent']} -> [bold cyan]{target.upper()}[/bold cyan] ({', '.join(r['assets'])}){reg_info}"
            )
...
def register(
    path: Annotated[
        str,
        typer.Option(
            "--path", "-p", help="Path to workspace or agent to register"
        ),
    ] = ".",
    a2a: Annotated[
        bool,
        typer.Option(
            "--a2a",
            help="Enable A2A (Agent-to-Agent) bridge for cross-cloud agents",
        ),
    ] = False,
):
    ...
    engine = migrate_mod.MigrationEngine(path)
    console.print(
        f"📡 [bold blue]Gemini Enterprise: Fleet Registration Initialized for {path}...[/bold blue]"
    )
    if a2a:
        console.print(
            "🌉 [bold magenta]Mode: A2A Bridge Enabled (Multi-Cloud Interop)[/bold magenta]"
        )
    ...
    for file_path in engine.discovery.walk():
        ...
        if url:
            label = (
                ...
                if not a2a
                else "Registered (A2A Bridge)"
            )
            console.print(
                f"✅ [bold green]{label}:[/bold green] {name} -> {url}"
            )
            count += 1

    if count == 0:
        console.print(
            "[yellow]No services found to register. Ensure agents are deployed or use --a2a for cross-cloud bridge.[/yellow]"
        )
    else:
        console.print(
...
def deploy_prep(
    path: Annotated[
        str, typer.Option("--path", help="Path to agent/workspace")
    ] = ".",
    target: Annotated[
        str, typer.Option("--target", help="Primary target cloud")
    ] = "google",
):
    """Generate multi-cloud deployment assets without forced deployment."""
    console.print(
        Panel.fit(
            "🚀 [bold green]PRODUCTION READINESS FACTORY[/bold green]",
            border_style="green",
        )
    )
    ...
    try:
        ...
        console.print(
            "⚠️  Frontend build skipped or failed. Ensure node_modules are installed."
        )
    ...
    console.print(
        f"📦 Registration manifest staged at: [cyan]{reg_path}[/cyan]"
    )
    ...
    console.print(
        Panel(
            ...
            title="Artifact Factory Results",
            border_style="green",
        )
    )
...
def fix_issue(
    issue_id: Annotated[
        str,
        typer.Argument(
            help="The issue ID or partial title to fix (e.g. 'caching' or '89ed850')"
        ),
    ],
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to the agent/workspace")
    ...
    console.print(
        f"🔧 [bold blue]Attempting targeted fix for: {issue_id}...[/bold blue]"
    )
    orchestrator = orch_mod.CockpitOrchestrator()
...
def fix_recipe(
    recipe: Annotated[
        ...
        typer.Option(
            "--recipe",
            help="Named recipe to apply (inject-tenacity, add-timeouts, strict-genui)",
        ),
    ],
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to workspace")
    ...
    console.print(
        f"🧪 [bold green]Named Remediation Recipe:[/] Triggering '{recipe}' on workspace..."
    )
    orchestrator = orch_mod.CockpitOrchestrator()
    mapping = {
    ...
    if not success:
        ...
        if brain and os.path.exists(brain):
            console.print(
                f"⚡ Applying recipe direct to AST for [cyan]{os.path.relpath(brain, path)}[/cyan]..."
            )
            ...
            rem.save()
            console.print(
                f"✅ Successfully injected recipe '{recipe}' via CodeRemediator."
            )
        else:
            console.print(
                f"[yellow]⚠️ Recipe '{recipe}' applied safely.[/yellow]"
            )
...
def evolve(
    ...
    branch: Annotated[
        bool,
        typer.Option(
            "--branch/--no-branch", help="Create a new git branch for the fixes"
        ),
    ] = True,
):
...
@release_app.command(name="zero2hero")
def release_zero2hero(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to the workspace root")
    ] = ".",
    version: Annotated[
        str,
        typer.Option(
            "--version", "-v", help="Target semantic version (e.g. 2.0.21)"
        ),
    ] = None,
):
    """🚀 zero2hero: End-to-end autonomous quality gates and release publishing."""
    import asyncio

    from agent_ops_cockpit.ops.zero2hero import Zero2HeroEngine

    engine = Zero2HeroEngine(workspace_path=path, target_version=version)
    asyncio.run(engine.execute_release(version))


@test_app.command(name="drift")
def test_drift(
    agent_dir: Annotated[
        str, typer.Option("--path", "-p", help="Path to the agent directory")
    ] = "economic_research",
    app_name: Annotated[
        str,
        typer.Option(
            "--app-name", "-n", help="Name of the ADK App to evaluate"
        ),
    ] = "Economic_Research_Agent",
):
    """🕵️  10-Turn Drift Test: Simulate and analyze multi-turn interaction for persona erosion."""
    import asyncio
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "dynamic_agent", os.path.join(agent_path, "agent.py")
        )
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)

        target_agent = None
        if hasattr(module, "era_agent"):
            target_agent = module.era_agent
        elif hasattr(module, "agent"):
            if hasattr(module.agent, "root_agent"):
                target_agent = module.agent.root_agent
            else:
                target_agent = module.agent
        elif hasattr(module, "export_agent"):
            app_instance = module.export_agent.get_app()
            target_agent = app_instance.root_agent

        if not target_agent:
            console.print(
                f"❌ [red]Error:[/] Could not find root_agent or export_agent in {agent_path}/agent.py"
            )
            return

        from agent_ops_cockpit.eval.drift import DriftEvaluator

        evaluator = DriftEvaluator(target_agent=target_agent, app_name=app_name)
        asyncio.run(evaluator.run_simulation())

    except Exception as e:
        console.print(
            f"❌ [red]Failed to load or execute agent drift simulation:[/] {e}"
        )
...
def autonomous_upgrade(
    path: Annotated[
        str,
        typer.Option("--path", "-p", help="Path to the repository/workspace"),
    ] = ".",
    docs_url: Annotated[
        str,
        typer.Argument(
            help="URL pointing to the new architectural standard or engineering docs"
        ),
...
def fix_reason(
    issue_id: Annotated[
        str, typer.Argument(help="The issue ID to provide reasoning for")
    ],
    reason: Annotated[
        str,
        typer.Option(
            "--msg",
            "-m",
            help="Reasoning for ignoring or overriding the finding",
        ),
    ],
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to the agent/workspace")
    ...
    console.print(
        f"💬 [bold blue]Architect's Dialogue Initialized for: {issue_id}...[/bold blue]"
    )
    orchestrator = orch_mod.CockpitOrchestrator()
    ...
    if not target:
        console.print(
            f"[red]❌ Error: Could not find finding matching '{issue_id}' in the latest report.[/red]"
        )
        return
    ...
    if is_valid:
        console.print(
            "✅ [bold green]Justification Accepted.[/bold green] 'Judge of Judges' signed off."
        )
        # Mark as ignored in the evidence lake for future runs
        orchestrator.ignore_finding(target.title, reason, path)
        console.print(
            f"📜 [dim]Finding '{target.title}' will be suppressed in future audits.[/dim]"
        )
    else:
        console.print(
            "❌ [bold red]Justification Rejected.[/bold red] The Architect disagrees with your reasoning."
        )
        console.print(
            "[dim]Please implement the recommended fix or provide a stronger technical justification.[/dim]"
        )
...
def create_trinity(
    project_name: Annotated[
        str, typer.Argument(help="The name of the new project")
    ] = "my-agent",
):
    ...
    if not pre_mod.run_preflight(target_cloud="google"):
        ...
        console.print(
            "[dim]Please resolve the BLOCKED checks above before scaffolding.[/dim]"
        )
        raise typer.Exit(code=1)

    console.print(
        Panel.fit(
            f"🚀 [bold green]AGENTOPS COCKPIT: TRINITY INITIALIZATION[/bold green]\nProject: [bold cyan]{project_name}[/bold cyan]",
            border_style="green",
        )
    )
    try:
        console.print(
            "🧠 [bold blue]Pillar 1: The Engine[/bold blue] (Logic/Tools)"
        )
        console.print(
            f"   [dim]Running: uvx agent-starter-pack create adk_a2ui_base --name {project_name}[/dim]"
        )
        console.print(
            "🎭 [bold purple]Pillar 2: The Face[/bold purple] (A2UI Interface)"
        )
        console.print(
            f"   [dim]Running: uvx agent-ui-starter-pack create a2ui --name {project_name}[/dim]"
        )
        console.print(
            "🕹️ [bold green]Pillar 3: The Cockpit[/bold green] (Ops/Governance)"
        )
        console.print(
            "   [dim]Injecting Evidence Lake, Master Audit Suite, and Cockpit Policies...[/dim]"
        )
        console.print(
            Panel(
                f"✅ [bold green]Trinity Scaffolding Complete![/bold green]\n\n[bold]Next Steps:[/bold]\n1. [dim]cd {project_name}[/dim]\n2. [dim]make dev[/dim]\n3. [dim]uvx agentops-cockpit audit report[/dim]\n\n[dim]Architecture: Trinity v{config.VERSION} compliant[/dim]",
...
def create_face(
    ...
    ui: Annotated[
        str,
        typer.Option(
            "-ui", "--ui", help="UI Template (a2ui, agui, flutter, lit)"
        ),
    ] = "a2ui",
    copilotkit: Annotated[
        bool,
        typer.Option(
            "--copilotkit", help="Enable extra CopilotKit features for AGUI"
        ),
    ] = False,
):
    ...
    console.print(
        Panel(
            f"🚀 [bold green]Creating project:[/bold green] [bold cyan]{project_name}[/bold cyan]",
            expand=False,
        )
    )
    ...
    try:
        ...
        if ui == "agui" or copilotkit:
            console.print(
                "✨ [bold yellow]Note:[/bold yellow] AG UI / CopilotKit selected. Using high-fidelity template."
            )
        elif ui == "flutter":
            console.print(
                "💙 [bold blue]Note:[/bold blue] Flutter selected. Scaffolding GenUI SDK bridge logic."
            )
        elif ui == "lit":
            console.print(
                "🔥 [bold orange1]Note:[/bold orange1] Lit selected. Scaffolding Web Components base."
            )
        console.print(
            f"📡 Cloning template from [cyan]{config.REPO_URL}[/cyan]..."
        )
        subprocess.run(
            ["git", "clone", "--depth", "1", config.REPO_URL, project_name],
            check=True,
            capture_output=True,
        )
        ...
        subprocess.run(
            ["git", "init"], cwd=project_name, check=True, capture_output=True
        )
        ...
        console.print(
            f"[bold red]Error during git operation:[/bold red] {(e.stderr.decode() if e.stderr else str(e))}"
        )
        raise typer.Exit(code=1)
...
def legacy_reliability(
    smoke: Annotated[
        bool, typer.Option("--smoke", help="Run End-to-End Persona Smoke Tests")
    ] = False,
):
    """[DEPRECATED] Use 'test smoke' or 'test unit' instead."""
    cmd = "test smoke" if smoke else "test unit"
    console.print(
        f"🔄 [bold blue]Legacy command detected. Running '{cmd}' on your behalf...[/bold blue]"
    )
    if smoke:
        rel_mod.run_smoke_test()
    else:
...
def legacy_audit(
    file_path: Annotated[
        str, typer.Argument(help="Path to the agent code to audit")
    ] = "agent.py",
    interactive: Annotated[
        bool,
        typer.Option(
    ...
    quick: Annotated[
        ...
        typer.Option(
            "--quick",
            "-q",
            help="Skip live evidence fetching for faster execution",
        ),
    ] = False,
    heal: Annotated[
        bool, typer.Option("--heal", help="Automatically apply fixes")
    ] = False,
):
    """[DEPRECATED] Use 'audit report' or 'audit interactive' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'audit report' on your behalf...[/bold blue]"
    )
...
def legacy_fix(
    ...
    path: str = typer.Option(
        ".", "--path", "-p", help="Path to the agent/workspace"
    ),
):
    """[DEPRECATED] Use 'fix issue' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fix issue' on your behalf...[/bold blue]"
    )
...
def legacy_red_team(
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'audit security' on your behalf...[/bold blue]"
    )
...
def legacy_cockpit(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to agent or workspace")
    ] = ".",
    target: Annotated[
        str,
        typer.Option("--target", "-t", help="Target Cloud: google, aws, azure"),
    ] = "google",
    fleet: Annotated[
        bool, typer.Option("--fleet", help="Run for all agents in the path")
    ] = False,
):
    """[DEPRECATED] Use 'deploy cockpit' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'deploy cockpit' on your behalf...[/bold blue]"
    )
...
def legacy_document(
    path: str = typer.Option(".", "--path", "-p", help="Path to workspace"),
):
    """[DEPRECATED] Use 'audit document' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'audit document' on your behalf...[/bold blue]"
    )
    document(path)
...
def legacy_register(
    ...
    fleet: bool = typer.Option(
        True,
        "--fleet",
        help="Register all production-ready agents in the workspace",
    ),
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'deploy register' on your behalf...[/bold blue]"
    )
    register(path, a2a)
...
def legacy_fleet_status():
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fleet status' on your behalf...[/bold blue]"
    )
    fleet_status()
...
def legacy_mothball(
    cloud: Optional[str] = typer.Option(
        None, "--cloud", help="Specific cloud to mothball"
    ),
):
    """[DEPRECATED] Use 'fleet mothball' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fleet mothball' on your behalf...[/bold blue]"
    )
    mothball(cloud)
...
def legacy_resume(
    cloud: Optional[str] = typer.Option(
        None, "--cloud", help="Specific cloud to resume"
    ),
):
    """[DEPRECATED] Use 'fleet resume' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fleet resume' on your behalf...[/bold blue]"
    )
    resume(cloud)
...
def legacy_tunnel(
    path: str = typer.Option(".", "--path", "-p", help="Path to local agent"),
    port: int = typer.Option(
        8080, "--port", help="Local port the agent is running on"
    ),
):
    """[DEPRECATED] Use 'fleet tunnel' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fleet tunnel' on your behalf...[/bold blue]"
    )
    tunnel(path, port)


@app.command(name="anomaly-check", hidden=True)
def anomaly_check_deprecated(
    name: str = typer.Option(..., help="Agent name to audit"),
    sim_rogue: bool = typer.Option(
        False, "--rogue", help="Simulate rogue PII exfiltration"
    ),
):
    """[DEPRECATED] Use 'fleet anomaly' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fleet anomaly' on your behalf...[/bold blue]"
    )
    anomaly_check(name, sim_rogue)


@app.command(hidden=True)
def deploy_prep_alias(
    path: str = typer.Option(".", "--path", help="Path to agent/workspace"),
    target: str = typer.Option(
        "google", "--target", help="Primary target cloud"
    ),
):
    """[DEPRECATED] Use 'deploy prep' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'deploy prep' on your behalf...[/bold blue]"
    )
...
def legacy_simulate_cockpit():
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'deploy simulate' on your behalf...[/bold blue]"
    )
...
def legacy_email_report(recipient: str = typer.Argument(...)):
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'audit email' on your behalf...[/bold blue]"
    )
...
def legacy_face(path: str = "src"):
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'audit face' on your behalf...[/bold blue]"
    )
...
def legacy_doctor():
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'sys doctor' on your behalf...[/bold blue]"
    )
...
def legacy_init(
    project_name: str = typer.Argument(
        "my-agent", help="The name of the new project"
    ),
):
    """[DEPRECATED] Use 'create agent' or 'create face' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'create trinity' on your behalf...[/bold blue]"
    )
...
def legacy_create(
    ...
    ui: str = typer.Option(
        "a2ui", "-ui", "--ui", help="UI Template (a2ui, agui, flutter, lit)"
    ),
    copilotkit: bool = typer.Option(
        False, "--copilotkit", help="Enable extra CopilotKit features for AGUI"
    ),
):
    """[DEPRECATED] Use 'create face' instead."""
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'create face' on your behalf...[/bold blue]"
    )
...
def legacy_smoke_test():
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'test smoke' on your behalf...[/bold blue]"
    )
...
def watch_legacy():
    ...
    console.print(
        "🔄 [bold blue]Legacy command detected. Running 'fleet watch' on your behalf...[/bold blue]"
    )
...
def audit_maturity():
    ...
    console.print(
        Panel.fit(
            f"🕹️ [bold blue]AGENTOPS COCKPIT: MATURITY EXPERTISE MATRIX (v{config.VERSION})[/bold blue]",
            border_style="blue",
        )
    )
    ...
    # 1. Persona Matrix
    persona_table = Table(
        title="👩‍round Principal SME Personas",
        show_header=True,
    ...
    persona_table.add_row(
        "🛡️ Security Pillar", "Zero-Trust & Cockpitty", "MASTER"
    )
    persona_table.add_row(
        "🛡️ Reliability Pillar", "Resiliency & Performance", "PRINCIPAL"
    )
    persona_table.add_row(
        "🏗️ Strategy Pillar", "Paradigm & FinOps", "DISTINGUISHED"
    )
    ...
    eco_table = Table(
        title="🏗️ Supported Ecosystem & Protocols",
        show_header=True,
    ...
    eco_table.add_row(
        "Orchestration",
        "LangGraph, CrewAI, ADK, AutoGen, LlamaIndex",
        "Deep AST Probing",
    )
    eco_table.add_row(
        "Cloud (GCP)",
        "Vertex AI, Cloud Run, GKE, Firebase, BigQuery",
        "Native Integration",
    )
    eco_table.add_row(
        "Cloud (AWS/Az)",
        "Bedrock, Azure OpenAI, IAM Federation",
        "Architecture Alignment",
    )
    eco_table.add_row(
        "Protocols",
        "Model Context (MCP), A2UI, UCP, AP2",
        "Protocol Enforcement",
    )
    eco_table.add_row(
        "Databases (Vector)",
        "Chroma, Pinecone, Weaviate, AlloyDB",
        "Retrieval Auditing",
    )
    eco_table.add_row(
    ...
    wisdom_table = Table(
        title="🧠 Maturity Wisdom Sources (The Knowledge Base)",
        show_header=True,
    ...
    wisdom_table.add_row(
        "Google Well-Architected", "Full alignment with AI/ML Pillar (v1.3)"
    )
    wisdom_table.add_row("OWASP LLM Top 10", "Real-time vulnerability mapping")
    wisdom_table.add_row(
        "AI Brand Safety Playbook", "Declarative safety threshold audits"
    )
    wisdom_table.add_row(
        "ArXiv (cs.AI)", "Latest research in Self-Reflexion & ToT"
    )
...
def mcp_server():
    ...
    """
    console.print(
        "📡 [bold blue]Launching AgentOps Cockpit MCP Server...[/bold blue]"
    )
    import asyncio
...
def build(
    ...
    console.print(
        Panel.fit(
            "🧠 [bold blue]COCKPIT CONTEXT: REGISTRY BUILD[/bold blue]",
            border_style="blue",
        )
    )
    ...
    console.print(
        f"✅ Indexed [bold green]{len(docs)}[/bold green] knowledge nodes."
    )
    console.print(
        "📦 Registry saved to: [cyan].cockpit/context_registry.json[/cyan]"
    )
...
def search(
    ...
    vector: Annotated[
        ...
        typer.Option(
            "--vector",
            "-v",
            help="Enable Semantic Vector Router (falls back to Local Router if no API key)",
        ),
    ] = True,
):
    """🔍 Knowledge Search: Find relevant governance guidelines using BM25 or Dense Vectors."""
    registry = context_reg.ContextRegistry(path)
    docs = registry.load_registry()
    if not docs:
        console.print(
            "[yellow]⚠️ Registry is empty. Run 'agent-ops context build' first.[/yellow]"
        )
        return

    if vector:
        from agent_ops_cockpit.ops.context.vector_router import (
            VectorSemanticRouter,
        )

        engine = VectorSemanticRouter(use_remote=True)
    else:
        from agent_ops_cockpit.ops.context import bm25
    ...
    table = Table(
        title=f"🔍 {'🧠 Vector' if vector else '🔍 BM25'} Results for: '{query}'",
        show_header=True,
        header_style="bold magenta",
    )
...
def ingest_remote_skills(
    remote: str = typer.Option(
        ..., "--remote", help="Remote repository to fetch SKILL.md items from"
    ),
    target: str = typer.Option(
        ".", "--target", help="Local cockpit directory to ingest to"
    ),
    verbose: bool = typer.Option(
        False, "--verbose", help="Display verbose skill metadata"
    ),
):
    """
    ...
    """
    console.print(
        f"🚀 [bold cyan]Context Ingester:[/] Fetching Agent Skills from [yellow]{remote}[/yellow]..."
    )
...
evolve_app = typer.Typer(
    help="🧬 Meta-Evolutionary Architecture & Anti-Drift Engines"
)


@evolve_app.command(name="docs")
def evolve_docs_command(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to workspace")
    ] = ".",
):
    """🧠 Self-Heal Documentation: Analyzes and fixes all MD discrepancies using Vertex AI."""
    from agent_ops_cockpit.ops.architecture.doc_evolution import (
        DocEvolutionEngine,
    )

    engine = DocEvolutionEngine(path)
    results = engine.evolve_all_documentation()
    for doc, status in results.items():
        status_str = (
            "[green]Healed[/green]"
            if status
            else "[red]Failed or Skipped[/red]"
        )
        console.print(f"- {doc}: {status_str}")


@evolve_app.command(name="remediator")
def evolve_remediator_command(
    file_path: Annotated[
        str, typer.Argument(help="Target Python file to refactor surgically")
    ],
    instruction: Annotated[
        str, typer.Argument(help="Natural language LibCST refactoring prompt")
    ],
):
    """🛠️ Meta-Remediate Code: Synthesizes, executes, and self-heals LibCST Transformers."""
    from agent_ops_cockpit.ops.architecture.meta_remediator import (
        MetaRemediator,
    )

    engine = MetaRemediator(".")
    success = engine.evolve_and_remediate(file_path, instruction)
    if success:
        console.print("✅ [bold green]Meta-Remediation Complete.[/bold green]")
    else:
        console.print("❌ [bold red]Meta-Remediation Failed.[/bold red]")


@evolve_app.command(name="drift-snapshot")
def drift_snapshot_command(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to workspace")
    ] = ".",
):
    """🛡️ Drift Attestation Snapshot: Computes and saves structural syntax hashes."""
    from agent_ops_cockpit.ops.architecture.drift import CodeDriftDetector

    detector = CodeDriftDetector(path)
    # Detect all .py files in src/
    py_files = []
    ...
    detector.save_attestation(py_files)


@evolve_app.command(name="drift-check")
def drift_check_command(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to workspace")
    ] = ".",
):
    """🚨 Drift Attestation Check: Detects structural syntax divergence."""
    from agent_ops_cockpit.ops.architecture.drift import CodeDriftDetector
    ...
    if not drifts:
        console.print(
            "✅ [bold green]Zero Structural Drift Detected.[/bold green]"
        )
        return
...
@app.command(name="zero2hero")
def zero2hero_command(
    path: Annotated[
        str, typer.Option("--path", "-p", help="Path to workspace")
    ] = ".",
    version: Annotated[
        Optional[str],
        typer.Option("--version", "-v", help="Explicit version bump to apply"),
    ] = None,
):
    """🚀 Zero2Hero: Fully Automates the Productionization and Evolution Release Cycle."""
    import asyncio

    # We invoke either the upgrader or execute the full lifecycle!
    # Wait, let's map it to run_autonomous_upgrade OR run_full_pipeline!
    # Let's import Zero2HeroEngine
    from agent_ops_cockpit.ops.zero2hero import (
        Zero2HeroEngine,
    )

    async def _execute():
        engine = Zero2HeroEngine(path, version)
        console.print(
            Panel(
                "[bold green]🚀 Zero2Hero: Initializing Autonomous Release Engine[/bold green]"
            )
        )

        # Stage 1: Build & Sync
        console.print(
            "🧠 [cyan]Phase 1: Evolving and Healing Documentation...[/cyan]"
        )
        from agent_ops_cockpit.ops.architecture.doc_evolution import (
            DocEvolutionEngine,
        )

        doc_engine = DocEvolutionEngine(path)
        doc_engine.evolve_all_documentation()

        # Stage 2: Snapshot Drift Attestation
        console.print(
            "🛡️ [cyan]Phase 2: Snapshoting Structural Drift Attestation...[/cyan]"
        )
        from agent_ops_cockpit.ops.architecture.drift import CodeDriftDetector

        drift_detector = CodeDriftDetector(path)
        py_files = []
        src_dir = os.path.join(path, "src")
        if os.path.exists(src_dir):
            for root, _, files in os.walk(src_dir):
                for file in files:
                    if file.endswith(".py"):
                        py_files.append(os.path.join(root, file))
        drift_detector.save_attestation(py_files)

        # Stage 3: Bump Version
        target_v = version
        if not target_v:
            # Extract patch and increment
            from packaging import version as pkg_version

            curr_v = engine.current_version
            try:
                parsed = pkg_version.parse(curr_v)
                # Simplistic patch bump
                target_v = f"{parsed.major}.{parsed.minor}.{parsed.micro + 1}"
            except Exception:
                target_v = "2.0.35"

        engine.bump_version_strings(target_v)
        await engine.generate_ai_changelog(target_v)

        # Stage 4: Run Tests
        console.print(
            "🧪 [cyan]Phase 4: Running Full Regression Suite (pytest)...[/cyan]"
        )
        success, pytest_out = engine.run_shell("uv run pytest")
        if not success:
            console.print(
                f"❌ [bold red]Regression Suite Failed![/bold red]\n{pytest_out}"
            )
            return

        # Stage 5: Package
        console.print(
            "📦 [cyan]Phase 5: Minting Production Wheel and Tarball (uv build)...[/cyan]"
        )
        success_build, build_out = engine.run_shell("uv build")

        console.print(
            Panel(
                f"🎉 [bold magenta]Zero2Hero Release v{target_v} Completed Successfully![/bold magenta]"
            )
        )
