...
app = typer.Typer(
    help="AgentOps Cockpit: The AI Agent Operations Platform", no_args_is_help=True
)
console = Console()
...
def policy_audit(
    input_text: str = typer.Option(
        None, "--text", "-t", help="Input text to validate against policies"
    ),
):
    """
    ...
    if input_text:
        try:
            ...
            console.print(
                "✅ [bold green]Input Passed Guardrail Validation.[/bold green]"
            )
        except policy_mod.PolicyViolation as e:
            console.print(
                f"❌ [bold red]Policy Violation Detected:[/bold red] {e.category} - {e.message}"
            )
    else:
        report = engine.get_audit_report()
        console.print(
            f"📋 [bold cyan]Policy Engine Active:[/bold cyan] {report['policy_active']}"
        )
        console.print(
...
def red_team(
    agent_path: str = typer.Argument(
        "src/agent_ops_cockpit/agent.py", help="Path to the agent code to audit"
    ),
):
    """
...
def load_test(
    url: str = typer.Option(
        "http://localhost:8000/agent/query?q=healthcheck", help="URL to stress test"
    ),
...
def deploy(
    service_name: str = typer.Option(
        "agent-ops-backend", "--name", help="Cloud Run service name"
    ),
    ...
    console.print(
        Panel.fit(
            "🚀 [bold green]AGENT COCKPIT: 1-CLICK DEPLOY[/bold green]",
            border_style="green",
        )
    )
...
def email_report(recipient: str = typer.Argument(..., help="Recipient email address")):
    ...
    if not os.path.exists("cockpit_final_report.md"):
        console.print(
            "[red]❌ Error: No audit report found. Run 'agent-ops report' first.[/red]"
        )
        return
...
def diagnose():
    ...
    console.print(
        Panel.fit(
            "🩺 [bold blue]AGENTOPS COCKPIT: SYSTEM DIAGNOSIS[/bold blue]",
            border_style="blue",
        )
    )
    ...
    if found_keys:
        table.add_row(
            "LLM API Keys",
            f"[green]FOUND ({len(found_keys)})[/green]",
            f"Detected: {', '.join([k.split('_')[0] for k in found_keys])}",
        )
    else:
        table.add_row(
...
def create(
    ...
    console.print(
        Panel(
            f"🚀 Creating project: [bold cyan]{project_name}[/bold cyan]", expand=False
        )
    )
