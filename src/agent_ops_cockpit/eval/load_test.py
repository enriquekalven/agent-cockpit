...
async def fetch(session, url, semaphore, results, progress, task_id):
    async with semaphore:
        ...
        try:
            ...
            results.append(
                {"status": "Error", "latency": time.time() - start, "error": str(e)}
            )
        finally:
...
async def run_load_test(url: str, requests: int, concurrency: int):
    ...
    console.print(
        f"Total Requests: [bold]{requests}[/bold] | Concurrency: [bold]{concurrency}[/bold]\n"
    )
    ...
    with Progress(
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                fetch(session, url, semaphore, results, progress, task_id)
...
def display_results(results):
    ...
    table.add_row(
        "Success Rate",
        f"{(len(successes) / len(results)) * 100:.1f}%" if results else "0%",
        "> 99%",
    )
    table.add_row(
        "Avg Latency",
