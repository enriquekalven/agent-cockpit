...
app = typer.Typer(
    help="AgentOps Load Tester: Stress test your agent endpoints."
)
console = Console()
...
async def validate_endpoint(url: str) -> bool:
    ...
    console.print(
        f"🕵️  [bold blue]Endpoint Handshake:[/] Verifying [cyan]{url}[/cyan]..."
    )
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=5) as response:
                ...
                if "text/html" in content_type:
                    console.print(
                        "⚠️  [bold yellow]HANDSHAKE WARNING:[/bold yellow] Target returned HTML instead of API data. This looks like a dashboard, not an agent."
                    )
                    return False
...
def display_results(results) -> bool:
    ...
    latencies = [
        r["latency"] for r in results if isinstance(r["latency"], (int, float))
    ]
    ...
    table.add_row(
        "Avg Latency",
        f"{sum(latencies) / len(latencies):.3f}s" if latencies else "N/A",
        "< 2.0s",
    )
    ...
    table.add_row(
        "Total Errors", f"[{error_color}]{len(errors)}[/{error_color}]", "0"
    )
    console.print("\n")
    console.print(table)
...
def run(
    url: str = typer.Option(
        "https://agent-cockpit.web.app/", help="URL to stress test"
    ),
    requests: int = typer.Option(50, help="Total number of requests"),
    concurrency: int = typer.Option(
        5, help="Simultaneous requests (Concurrent Users)"
    ),
):
    """
    ...
    try:
        ...
        if not is_valid:
            console.print(
                "⚠️  [bold yellow]Proceeding with load test anyway in case of override support...[/bold yellow]"
            )
