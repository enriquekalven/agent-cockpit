...
def main():
    ...
    console.print(
        "🚀 [bold blue]SKILL-BASED RED TEAMING INITIALIZED[/bold blue]"
    )

    skills_dir = os.path.join(
        os.path.abspath(target_path), ".cockpit", "promptfoo_skills"
    )
    if not os.path.exists(skills_dir):
        console.print(
            f"ℹ️ No skill-based Promptfoo configs found at {skills_dir}"
        )
        return
    ...
    if not configs:
        console.print(
            f"ℹ️ No skill-based Promptfoo configs found in {skills_dir}"
        )
        return
    ...
    for config_file in configs:
        ...
        console.print(
            f"🚀 [bold blue]Mocking Promptfoo for {skill_name}...[/bold blue]"
        )
        ...
        dummy_results = {
            "summary": {"numPassed": num_tests, "numTests": num_tests},
            "results": [],
        }
        ...
        for res in data.get("results", []):
            if not res.get("success"):
                ...
                print(
                    f"ACTION: {target_path} | Skill Breach: {skill_name} | Agent failed skill check for prompt: {prompt}"
                )

    console.print("\n", summary_table)

    overall_score = (
        int((total_passed / total_tests) * 100) if total_tests > 0 else 0
    )
    console.print(
