...
app = typer.Typer(
    help="Agent Quality Hill Climber: Iteratively optimize agent quality using ADK patterns."
)
console = Console()
GOLDEN_DATASET = [
    {
    ...
    {
        "query": "What is the distributed cache?",
        "expected": "The distributed cache is a semantic caching layer for reducing LLM costs.",
        "type": "definition",
    },
    {
...
class QualityJudge:
    ...
    async def score_response(
        actual: str, expected: str, metric: str = "similarity"
    ) -> float:
...
async def run_iteration(iteration: int, prompt_variant: str) -> dict:
    ...
    for item in dataset:
        ...
        match_score = await QualityJudge.score_response(
            actual_response, item["expected"]
        )
    ...
    reasoning_density = (
        avg_score / (tokens_used / 1000) if tokens_used > 0 else 0
    )
    return {
        "score": avg_score,
        "trajectory": avg_traj,
...
def climb(
    ...
    threshold: float = typer.Option(
        0.9, help="Target quality score (0.0 - 1.0)"
    ),
):
    """
    ...
    console.print(
        Panel.fit(
            "🧗 [bold cyan]QUALITY HILL CLIMBING v1.3: EVALUATION SCIENCE[/bold cyan]\nOptimizing Reasoning Density & Tool Trajectory Stability...",
            border_style="cyan",
        )
    )
    best_score = 0.75
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[yellow]Searching Reasoning Space...", total=steps
        )
        for i in range(1, steps + 1):
            progress.update(
                task, description=f"[yellow]Iteration {i}: Probing Gradient..."
            )
            ...
            if best_score >= threshold:
                ...
                break
    table = Table(
        title="📈 v1.3 Hill Climbing Optimization History",
    ...
    for h in history:
        ...
        table.add_row(
            ...
            h["status"],
            f"[{color}]+{h['improvement'] * 100:.1f}%[/{color}]"
            if h["improvement"] > 0
            else f"[red]{h['improvement'] * 100:.1f}%[/red]",
        )
    console.print(table)
    if best_score >= threshold:
        console.print(
