...
def run_shell_command(cmd: str, dir: str = ".") -> dict:
    ...
    try:
        result = subprocess.run(
            cmd, cwd=dir, shell=True, capture_output=True, text=True, check=True
        )
        return {
            "status": "success",
...
def invoke_meta_remediation(target_file_path: str, instruction: str) -> dict:
    """Uses Gemini and LibCST to synthesize, execute, and self-heal layout-preserving CST refactorings.
    Examples:
    - instruction="Inject 'timeout=30' into all httpx and generate_content calls."
    - instruction="Wrap all db connections with the @retry decorator and add required imports."
    """
    ...
    try:
        ...
        res = engine.evolve_and_remediate(target_file_path, instruction)
        return {
            "status": "success" if res else "error",
            "message": "Remediation applied surgically and tested via LibCST."
            if res
            else "Refactoring failed to modify file or synthesize correctly.",
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
Step 3: Refactor the code by invoking `invoke_meta_remediation` to apply perfectly formatted, layout-preserving Concrete Syntax Tree (CST) refactors to Python files. Alternatively, use `write_file` for new or configuration files.
Step 4: Verify your changes by running `run_shell_command` against the codebase tests (`uv run pytest`) or linters.
...
async def run_autonomous_upgrade(path: str, docs_url: str):
    ...
    console.print(
        "🚀 [bold blue]Cockpit Autonomous Upgrader Initializing...[/bold blue]"
    )
    console.print(
        f"📡 Target Path: [cyan]{path}[/cyan] | Standards Docs: [cyan]{docs_url}[/cyan]"
    )
    ...
    if portal.repo:
        ...
        console.print(
            "⚠️  [red]Failed to initialize Git Sandbox. Proceeding on current branch.[/red]"
        )
    ...
    console.print(
        "🤖 [bold green]Agent Dispatched![/bold green] Working autonomously in the background..."
    )
    console.print("=" * 60)
    ...
    async for event in runner.run_async(
        ...
        if hasattr(event, "is_tool_call") and event.is_tool_call():
            console.print(
                f"   [dim]🔧 Agent invoked tool:[/] [cyan]{event.tool_name}[/cyan]"
            )
    ...
    console.print("=" * 60)
    console.print(
        "✅ [bold green]Autonomous Refactoring Complete![/bold green]"
    )
    if portal.repo:
        console.print(
            f"👉 Review the differential in Git: [white]git diff main..{branch_name}[/white]"
        )
        console.print(
            f"👉 To adopt changes, merge the branch: [white]git checkout main && git merge {branch_name}[/white]"
        )
