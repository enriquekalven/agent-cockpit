...
class ShadowRunner:
    ...
    def run_differential(self):
        ...
        console.print(
            Panel.fit(
                "🕵️ [bold blue]COCKPIT: SHADOW MODE DIFFERENTIAL[/bold blue]",
                border_style="blue",
            )
        )
        ...
        console.print(
            f"🔸 [dim]Analyzing Candidate (v2): {self.candidate_path}[/dim]"
        )
        ...
        comparison = {
            ...
            "drift": [
                {
                    "query": "Handle missing API key",
                    "base_reasoning": "Raise Exception",
                    "candidate_reasoning": "Retry with exponential backoff + fallback to public-key",
                },
                {
                    "query": "Optimize loop",
                    "base_reasoning": "Standard for loop",
                    "candidate_reasoning": "Vectorized AST-based parallel map",
                },
            ],
        }
    ...
    def display_report(self, comp: dict):
        table = Table(
            title="📊 Reasoning & Performance Differential",
            show_header=True,
        ...
        drift_table = Table(
            title="🧠 Reasoning Drift Detected",
            show_header=True,
        ...
        for drift in comp["drift"]:
            drift_table.add_row(
                drift["query"],
                drift["base_reasoning"],
                drift["candidate_reasoning"],
            )
...
class ProductionShadowRouter:
    ...
    def __init__(self, diversion_percent: float = 0.05):
        ...
        console.print(
            f"🛰️ [bold green]Production Shadow Router Active.[/bold green] Diversion: [bold]{diversion_percent * 100}%[/bold]"
        )

    def route(
        self,
        user_input: str,
        base_agent_fn: callable,
        candidate_agent_fn: callable,
    ):
        """
        ...
        if should_shadow:
            try:
                ...
                self._record_differential(
                    user_input, base_response, candidate_response
                )
    ...
    def _record_differential(self, query: str, base_res: str, cand_res: str):
        ...
        trace_path = os.path.join(
            ...
            ".cockpit",
            "traces",
            f"diff_{datetime.now().strftime('%H%M%S')}.json",
        )
