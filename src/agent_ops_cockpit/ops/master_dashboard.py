...
Currently Tracking [bold green]{len(deployed_agents)}[/] Deployed Cloud Instances.
        ...
        console.print(
            Panel(hero_text, border_style="blue", box=box.DOUBLE_EDGE)
        )

        # 3. The 'Master Move' Panel (The Single Command)
        master_move = Panel(
            Text.from_markup(
                "🚀 [bold green]THE HERO ACTIONS:[/] [white]Run [bold]agentops-cockpit certify[/] to validate production-readiness or [bold]agentops-cockpit mcp blueprint[/] to modernize your tools.[/]"
            ),
            border_style="green",
        ...
        workflow_table = Table(
            title="🏛️ The 4-Step Cockpit Workflow", expand=True, box=box.ROUNDED
        )
        ...
        workflow_table.add_row(
            "4. Deploy", "deploy cockpit", "🚀 GKE / Cloud Run Production Push"
        )
        ...
        if deployed_agents:
            ...
            lake_path = os.path.join(
                os.getcwd(), ".cockpit", "evidence_lake.json"
            )
            lake_data = {}
            ...
            fleet_table = Table(
                title="🛰️ Active Cockpit Fleet Registry", expand=True
            )
            ...
            console.print(
                "[dim]No deployed agents detected in registry. Run 'agent-ops deploy register' or 'audit report' to begin.[/dim]"
            )
    ...
    def run_onboarding_guide(self):
        ...
        console.print(
            Panel.fit(
                "👋 [bold]Welcome to the Cockpit, Commander.[/]\nYou have inherited a fleet of autonomous agents. Here is your roadmap:",
