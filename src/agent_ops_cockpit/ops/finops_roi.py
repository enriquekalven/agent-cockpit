...
app = typer.Typer(
    help="FinOps ROI Waterfall: Model cost-per-task and suggest optimization pivots."
)
console = Console()
...
def waterfall(
    path: str = typer.Option(
        ".", "--path", "-p", help="Path to the agent project to analyze"
    ),
):
    """
    ...
    console.print(
        Panel.fit(
            "💰 [bold green]FINOPS ROI WATERFALL: TCO MODELING[/bold green]",
            border_style="green",
        )
    )
    ...
    if not findings:
        console.print(
            "ℹ️ [yellow]No explicit model identifiers found. Using default 'Gemini 1.5 Pro' baseline for waterfall simulation.[/yellow]"
        )
        findings.append(
            {
                "file": "Simulation",
    ...
    table.add_row(
        "1. Gemini 1.5 Flash Pivot",
        f"${baseline_cost:,.2f}",
        f"${flash_cost:,.2f}",
        f"-${baseline_cost - flash_cost:,.2f}",
    )

    # Phase 3: Context Caching
    caching_cost = flash_cost * 0.1  # 90% reduction
    table.add_row(
        "2. Context Caching (90%)",
        f"${flash_cost:,.2f}",
        f"${caching_cost:,.2f}",
        f"-${flash_cost - caching_cost:,.2f}",
    )
    ...
    console.print(
        Panel(
            f"🚀 [bold]Strategic ROI Summary[/bold]\nTotal monthly savings opportunity: [bold green]${total_savings:,.2f}[/bold green]\nROI Multiplier: [bold cyan]{(baseline_cost / caching_cost):.1f}x efficiency gain[/bold cyan]",
            border_style="blue",
        )
    )
