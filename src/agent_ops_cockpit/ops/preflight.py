...
class PreflightEngine:
    ...
    def check_registry_access(self, registry_url="https://pypi.org/simple"):
        ...
        try:
            ...
            if (
                ...
                console.print(
                    f"⚠️  [yellow]Registry Auth/Conn failure ({registry_url}). Attempting Resilient Failover...[/yellow]"
                )
                try:
                    with urllib.request.urlopen(
                        "https://pypi.org/simple", timeout=3
                    ) as response:
                        if response.status == 200:
                            ...
                            return (
                                True,
                                "Resilient Failover: Public PyPI mirrors active.",
                            )
                except Exception:
                    pass
            console.print(
                f"⚠️  [yellow]Registry check failed: {str(e)}. Proceeding with warning.[/yellow]"
            )
    ...
    def check_environment_consistency(self):
        ...
        env_files = [
            f for f in os.listdir(self.target_path) if f.startswith(".env")
        ]
        if not env_files:
            return (
                True,
                "No .env detected (assuming local simulation or IAM auth).",
            )
    ...
    def check_cloud_auth(self, target_cloud="google"):
        ...
        if target_cloud == "google":
            ...
            try:
                ...
                id_res = subprocess.check_output(
                ...
                return (
                    False,
                    "No AWS credentials found. Configure 'aws configure'.",
                )
            ...
            try:
                ...
                acc_res = subprocess.check_output(
        ...
        if target_cloud == "google":
            try:
                ...
                if not json.loads(services):
                    return (
                        False,
                        f"Vertex AI API (aiplatform) NOT enabled in {project_id}.",
                    )
                ...
                if billing.returncode != 0:
                    return (
                        False,
                        f"Billing check failed for {project_id}. Ensure Billing API is enabled.",
                    )
                ...
                if not billing_data.get("billingEnabled"):
                    return (
                        False,
                        f"Billing is DISABLED for project {project_id}.",
                    )

                return True, f"Project {project_id} (Billing OK, Vertex OK)"
            except Exception as e:
                console.print(
                    f"⚠️  [yellow]Cloud readiness check failed: {str(e)}. Proceeding with warning.[/yellow]"
                )
            ...
            try:
                ...
                if res.returncode != 0:
                    return (
                        False,
                        "Cannot list AWS Bedrock models. Check permissions.",
                    )
            ...
            try:
                ...
                if res.returncode != 0:
                    return (
                        False,
                        "Cannot list Azure Cognitive Services. Check subscription/permissions.",
                    )
    ...
    def optimize_dependencies_via_ast(self):
        ...
        try:
            ...
            console.print(
                f"📦 Detected needed packages: {', '.join(packages_to_install)}"
            )

            try:
                if os.path.exists(
                    os.path.join(self.target_path, "pyproject.toml")
                ):
                    subprocess.run(
                        ["uv", "add"] + list(packages_to_install),
                        check=True,
                        capture_output=True,
                    )
                else:
                    subprocess.run(
                        ["uv", "pip", "install"] + list(packages_to_install),
                        check=True,
                        capture_output=True,
                    )
                return (
                    True,
                    f"Installed detected packages: {', '.join(packages_to_install)}",
                )
            except Exception as e:
                return (
                    True,
                    f"Failed to install packages: {str(e)} (Proceeding anyway)",
                )
        except Exception as e:
    ...
    def run_all(self, target_cloud="google"):
        ...
        checks = [
            ...
            (
                "Cloud Cockpitty Auth",
                lambda: self.check_cloud_auth(target_cloud),
            ),
            (
                "Cloud Project Readiness",
                lambda: self.check_cloud_readiness(target_cloud),
            ),
        ]
        ...
        if not all_passed:
            console.print(
                "⚠️ [bold yellow]Pre-flight warnings detected. Some audit modules may exhibit drift.[/bold yellow]\n"
            )
        else:
            console.print(
                "✨ [bold green]Environment verified. Proceeding to Agent Audit.[/bold green]\n"
            )
