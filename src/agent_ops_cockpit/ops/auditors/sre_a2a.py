...
class SREAuditor(BaseAuditor):
    ...
    def audit(
        ...
        if (
            "pinecone" in content.lower() or "alloydb" in content.lower()
        ) and "grpc" not in content.lower():
        ...
        if "us-central1" in content.lower() and (
            "europe-west1" in content.lower() or "asia-east" in content.lower()
        ):
        ...
        if (
            ...
            and "redis" not in content.lower()
            and "memorystore" not in content.lower()
        ):
        ...
        if (
            "docker" in file_path.lower()
            or "deploy" in file_path.lower()
            or "yaml" in file_path.lower()
        ):
            if (
                "memory:" not in content.lower()
                and "cpu:" not in content.lower()
            ):
...
class InteropAuditor(BaseAuditor):
    ...
    def audit(
        ...
        if (
            "subprocess.run" in content.lower()
            or "requests.get" in content.lower()
        ):
        ...
        if "return" in content.lower() and "html" in content.lower():
            if (
                "surfaceid" not in content.lower()
                and "a2ui" not in content.lower()
            ):
