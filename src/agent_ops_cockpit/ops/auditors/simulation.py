...
class SimulationAuditor(BaseAuditor):
    ...
    def audit(
        ...
        prompts = re.findall(
            r"(instruction|prompt|system_message|system_instruction)\s*=\s*['\"]{3}([\s\S]+?)['\"]{3}",
            content,
            re.I,
        )
        ...
        for p_name, p_text in prompts:
            ...
            obey_count = len(
                re.findall(
                    r"(follow|obey|execute|do|perform|act|respond)",
                    p_text.lower(),
                )
            )
            guard_count = len(
                re.findall(
                    r"(never|prohibit|forbidden|stop|refuse|reject|sanitize|verify)",
                    p_text.lower(),
                )
            )
