                        ...
                        if not self._is_ignored(0, content, title):
                            findings.append(
                                AuditFinding(
                                    category="🎭 Behavioral",
                                    title=title,
                                    description=f"Code promises PII masking, but trace.json contains raw email patterns at {entry.get('timestamp')}.",
                                    impact="CRITICAL",
                                    roi="Ensure semantic masking logic handles 'suffix+alias' patterns correctly.",
                                    file_path=file_path,
                                )
                            )
        except Exception:
