...
class CockpitAuditor(BaseAuditor):
    ...
    VULNERABLE_PATTERNS = [
        (
            r"project[-_]id\s*=\s*['\"][\w-]+['\"]",
            "Hardcoded GCP Project ID. Use environment variables for portability.",
        ),
        (
            r"region\s*=\s*['\"]us-[\w-]+['\"]",
            "Hardcoded US Region. Risk to EU Data Cockpit if not configured per environment.",
        ),
        (
            r"aws_[\w-]+\s*=\s*['\"]",
            "AWS-specific credential detected. Ensure abstraction layer for Multi-Cloud.",
        ),
        (
            r"azure_[\w-]+\s*=\s*['\"]",
            "Azure-specific endpoint detected. Risk of vendor lock-in.",
        ),
    ]
    ...
    def audit(
        ...
        destructive_patterns = [
            r"def (tool_|_(trigger|delete|terminate|update|exec|scrape|charge))"
        ]
            ...
            for match in re.finditer(pat, content):
                ...
                for node in ast.walk(tree):
                    if (
                        ...
                        and node.lineno == line_no
                    ):
                        has_gate = any(
                            isinstance(d, (ast.Name, ast.Call))
                            and ("gate" in str(ast.dump(d)).lower())
