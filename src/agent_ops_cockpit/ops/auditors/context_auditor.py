...
class ContextSME:
    ...
    def audit(self, path: str):
        ...
        table.add_row(
            "System Instructions", "Static", "1,200", "[green]CACHED[/green]"
        )
        table.add_row(
            "Tool Definitions (MCP)", "Static", "3,500", "[green]CACHED[/green]"
        )
        table.add_row(
            "Conversation History",
            "Dynamic",
            "8,400",
            "[yellow]VOLATILE[/yellow]",
        )
        table.add_row(
            "RAG Snippets", "Dynamic", "15,000", "[red]UNOPTIMIZED[/red]"
        )
        ...
        console.print(
            "1. [bold]Context Caching[/bold]: Move RAG snippets to a cached preamble if reused across turns."
        )
        console.print(
            "2. [bold]Token Compression[/bold]: High density detected in 'Conversation History'. Consider summarization."
        )
