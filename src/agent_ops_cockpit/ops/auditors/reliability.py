    ...
    def audit(
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                await_nodes = [
                    n for n in ast.walk(node) if isinstance(n, ast.Await)
                ]
                    ...
                    if not self._is_ignored(node.lineno, content, title):
                        findings.append(
                            AuditFinding(
                                ...
                                title=title,
                                description="Multiple sequential 'await' calls identified. This increases total latency linearly.",
                                impact="MEDIUM",
            ...
            if isinstance(node, ast.Call):
                ...
                if any(
                    (
                        ...
                        for x in ["get", "post", "request", "fetch", "query"]
                    )
                ):
                    ...
                    if is_external:
                        ...
                        if not parent or (
                            not self._is_decorated_with_retry(parent)
                        ):
                            ...
                            if not self._is_ignored(
                                ...
                                findings.append(
                                    AuditFinding(
                                        ...
                                        title=title,
                                        description=f"External call '{func_name}' to '{node.args[0].value[:30]}...' is not protected by retry logic.",
                                        impact="HIGH",
                ...
                if isinstance(node.value, (ast.Constant, ast.JoinedStr)):
                    ...
                    if isinstance(node.value, ast.Constant) and isinstance(
                        ...
                        val = "".join(
                            [
                                s.value
                                for s in node.value.values
                                if isinstance(s, ast.Constant)
                            ]
                        )
                    if len(val) > 20 and any(
                        (
                            x in val.lower()
                            for x in ["act as", "you are", "instruction"]
                        )
                    ):
                        if not any(
                            (
                            ...
                            if not self._is_ignored(
                                ...
                                findings.append(
                                    AuditFinding(
                                        category="🧗 Reliability",
                                        title=title,
                                        description="System prompt lacks negative constraints (e.g., 'If you don't know, say I don't know').",
                                        impact="HIGH",
    ...
    def _is_decorated_with_retry(self, node):
        for decorator in node.decorator_list:
            if (
                ...
                and "retry" in decorator.id.lower()
            ):
                return True
            if (
                isinstance(decorator, ast.Call)
                and isinstance(decorator.func, ast.Name)
                and ("retry" in decorator.func.id.lower())
            ):
                return True
        return False
