    ...
    def audit(self, tree, content: str, file_path: str) -> List[AuditFinding]:
        ...
        if filename in ["package.json", "pyproject.toml", "requirements.txt"]:
            ...
            vulnerability_patterns = [
                (
                    r"langchain[<\s=]*0\.0",
                    "ASI-07: Legacy LangChain version detected. Susceptible to early-stage prompt injection vectors.",
                ),
                (
                    r"requests[<\s=]*2\.25",
                    "ASI-07: Vulnerable 'requests' version. High-risk CVE for agent-to-agent SSRF.",
                ),
                (
                    r"gpt-3\.5",
                    "FinOps: GPT-3.5-Turbo detected. This model is 'Legacy Intelligence' as of Feb 2026.",
                ),
                (
                    r"pickle|yaml\.load",
                    "ASI-03: Insecure Serializer detected in dependencies. Recommend switching to safer alternatives.",
                ),
            ]
                ...
                if re.search(pat, content, re.I):
                    findings.append(
                        AuditFinding(
                            ...
                            title="ASI-07: Vulnerable Supply Chain"
                            if "ASI" in reason
                            else "Legacy Intelligence SDK",
        ...
        if "mcp" in filename.lower() and filename.endswith(".json"):
            ...
            if (
                '"allowExecute": true' in content
                and '"restricted": true' not in content
            ):
                findings.append(
                    AuditFinding(
                        category="🛡️ SecOps",
