...
class ParadigmAuditor(BaseAuditor):
    ...
    def audit(
        ...
        """
        if not file_path.endswith(".py") or any(
            p in file_path
            for p in [
                "cli/commands",
                "cli/utils",
        ...
        is_potential_agent = (
            is_agent_logic or is_tool or file_path.endswith("agent.py")
        )
...
[bold green]RECOMMENDATION:[/bold green] Partition into specialist agents to improve focus.""",
        ...
        if (
            is_potential_agent
            and "requests" in imports
            and ("tool" in content_lower or "mcp" in content_lower)
        ):
            findings.append(
                AuditFinding(
                    category="🏗️ Strategy",
...
[bold green]RECOMMENDATION:[/bold green] Migrate to **Model Context Protocol (MCP)** for standardized tool reuse.""",
        ...
        if (
            is_potential_agent
            and "mcp" in content_lower
            and any(
                kw in content_lower for kw in ["client", "connect", "transport"]
            )
        ):
            if not any(
                kw in content_lower
                for kw in ["auth", "credential", "token", "secure", "header"]
            ):
                findings.append(
                    AuditFinding(
                        category="🏗️ Strategy",
...
[bold green]RECOMMENDATION:[/bold green] Implement **OIDC/OAuth Interceptors** for all MCP tool discovery.""",
        ...
        if (
            ("history" in content_lower or "messages" in content_lower)
            and "zep" not in content_lower
            and "memgpt" not in content_lower
            and "redis" not in content_lower
        ):
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **Persistent Memory (Zep, MemGPT, or Redis)** for long-term reasoning.""",
        ...
        if (
            is_potential_agent
            and any(
                re.search(rf"\b{kw}\b", content_lower)
                for kw in ["code", "legal", "finance", "medical", "test"]
            )
            and not any(
                kw in content_lower
                for kw in [
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **NL2SQL** or **Semantic Indexing**.""",
        ...
        if (
            ...
            and any(
                re.search(rf"\b{kw}\b", content_lower)
                for kw in ["math", "calculate", "sum", "arithmetic", "total"]
            )
            and any(
                re.search(rf"\b{kw}\b", content_lower)
                for kw in ["search", "retrieve", "knowledge_base"]
            )
        ):
            if (
                "sql" not in content_lower
                and "math" not in content_lower
                and "calculator" not in content_lower
            ):
                findings.append(
                    AuditFinding(
                        category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **Code Interpreter** or **SQL Agent**.""",
        ...
        if (
            ...
            and any(
                kw in content_lower
                for kw in ["prompt", "instructions", "message"]
            )
        ):
            if any(
                kw in content_lower
                for kw in ["transform", "clean", "extract", "parse", "format"]
            ) and (
                "sandbox" not in content_lower
                and "deterministic" not in content_lower
            ):
                findings.append(
                    AuditFinding(
                        category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to a **Local Parser** or **Code Interpreter**.""",
        ...
        if (
            is_potential_agent
            and ("os.walk" in content or "glob.glob" in content)
            and ("query" in content_lower or "search" in content_lower)
        ):
            findings.append(
                AuditFinding(
                    category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **Vector RAG (Pinecone/Chroma)**.""",
        ...
        if (
            any(
                kw in content_lower
                for kw in ["delete_", "write_", "execute_payment", "post_"]
            )
            and "approve" not in content_lower
            and "hitl" not in content_lower
            and "click." not in content_lower
            and "typer." not in content_lower
        ):
            findings.append(
                AuditFinding(
                    category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Implement **HITL Approval Nodes** (e.g., AG UI / CopilotKit).""",
        ...
        if any(
            kw in content_lower
            for kw in ["from_image", "from_uri", "image_url"]
        ) and not any(
            kw in content_lower
            for kw in ["safety", "modality_guard", "shield", "armor"]
        ):
            findings.append(
                AuditFinding(
                    category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Implement **SafetyPlugins** or **Model Armor** for multimodal streams.""",
        ...
        if (
            is_loop_llm
            and "langgraph" not in content_lower
            and "semantic_kernel" not in content_lower
        ):
            findings.append(
                AuditFinding(
                    category="🚀 Strategic Paradigm",
...
[bold purple]Architecture Suggestion:[/bold purple] Pivot to **LangGraph** to avoid reasoning collapse.""",
        ...
        if (
            ("response" in content_lower or "generation" in content_lower)
            and "stream=true" not in content_lower
            and "streaming" not in content_lower
        ):
            if any(
                kw in content_lower
                for kw in ["report", "summarize", "article", "write_"]
            ):
                findings.append(
                    AuditFinding(
                        category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **GenUI Streaming Protocol**.""",
        ...
        if (
            ...
            and "orchestrator" not in content_lower
        ):
            ...
            has_real_namespace = (
                re.search(r"[a-zA-Z0-9_-]+:[^:]", content_lower)
                or "/" in content_lower
            )
...
[bold green]RECOMMENDATION:[/bold green] Ensure every `Agent` in `sub_agents` has a high-fidelity `description` string.""",
        ...
        docstrings_len = sum(
            len(d) for d in re.findall(r'"""([\s\S]*?)"""', content)
        )
        if (
            docstrings_len > 10000
            and "finetune" not in content_lower
            and "distill" not in content_lower
        ):
            findings.append(
                AuditFinding(
                    category="🚀 Strategic Paradigm",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **Model Distillation**.""",
        ...
        if (
            any(
                kw in content_lower for kw in ["policy", "rules", "regulations"]
            )
            and "policy_engine" not in content_lower
            and "guardrail" not in content_lower
            and "typer" not in content_lower
            and "click" not in content_lower
        ):
            findings.append(
...
[bold green]RECOMMENDATION:[/bold green] Pivot to our **Centralized Policy Engine** or External Guardrails.""",
        ...
        if (
            ...
            and any(
                re.search(rf"\b{kw}\b", content_lower)
                for kw in ["delete", "pay", "transfer"]
            )
            and content_lower.count("if ") > 5
            and "policy" not in content_lower
            and "click." not in content_lower
            and "typer." not in content_lower
        ):
            findings.append(
...
[bold green]RECOMMENDATION:[/bold green] Migrate critical decision gates to the **cockpit Policy Engine**.""",
        ...
        if any(
            kw in content_lower for kw in ["complex task", "goal", "workflow"]
        ) and not any(
            kw in content_lower
            for kw in ["plan", "step_by_step", "react", "thought"]
        ):
            if (
                content_lower.count(" -> ") > 3
                or content_lower.count("then ") > 3
            ):
                findings.append(
                    AuditFinding(
                        category="🏗️ Strategy",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to a **Dynamic Planner** or **ReAct Pattern**.""",
        ...
        if is_potential_agent and any(
            re.search(rf"\b{kw}\b", content_lower)
            for kw in ["retrieve", "search"]
        ):
            if (
                not any(
                    kw in content_lower
                    for kw in ["conditional", "if confidence", "decide"]
                )
                and "typer" not in content_lower
                and "click" not in content_lower
            ):
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **Agentic/Active RAG** (retrieve only when needed).""",
        ...
        for f in findings:
            print(
                f"ACTION: {file_path}:{f.line_number or 1} | {f.title} | {f.roi}"
            )
        ...
        for value in node.values:
            if isinstance(value, ast.FormattedValue):
                if (
                    ...
                    and value.value.id in self.sources
                ):
            ...
            if isinstance(node.func, ast.Name) and node.func.id in [
                ...
                for kw in node.keywords:
                    if (
                        ...
                        and kw.value.id in self.sources
                    ):
