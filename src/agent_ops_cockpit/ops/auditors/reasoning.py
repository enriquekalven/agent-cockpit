...
class ReasoningAuditor(BaseAuditor):
    ...
    def audit(
        ...
        if re.search(
            r"gemini-3-pro|gpt-5\.2|claude-4\.6", content
        ) and re.search(r"for\s+\w+\s+in", content):
            if any(
                x in content.lower()
                for x in ["classify", "label", "is_", "has_"]
            ):
                title = "Model Efficiency Regression (v2.0.7)"
                if not self._is_ignored(0, content, title):
                    findings.append(
                        AuditFinding(
                            category="💰 FinOps",
        ...
        has_math_intent = any(
            re.search(pat, content.lower()) for pat in math_indicators
        )
        is_rag_active = any(
            x in content.lower()
            for x in ["vector", "retriever", "chroma", "pinecone", "search"]
        )
        ...
        runner_funcs = [
            ...
            and (n.name.startswith("run_") or n.name == "call_agent")
        ]
        for func in runner_funcs:
            has_reflection = any(
                isinstance(d, (ast.Name, ast.Call))
                and ("reflection" in str(ast.dump(d)).lower())
