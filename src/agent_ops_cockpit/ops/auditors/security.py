...
[bold green]RECOMMENDATION:[/bold green] Pivot to an **E2B Sandbox** for secure execution or use **DOMPurify** for UI rendering.""",
        ...
        if any(imp in content_lower for imp in crm_imports) and not any(
            kw in content_lower
            for kw in ["scrub", "mask", "pii", "anonymize", "guard", "redact"]
        ):
...
[bold green]RECOMMENDATION:[/bold green] Integrate the **Cockpit PII Scrubber** into the request pipeline.""",
        ...
        if ".env" in content_lower and (
            ...
            if (
                "secret_manager" not in content_lower
                and "vault" not in content_lower
            ):
...
[bold green]RECOMMENDATION:[/bold green] Migrate to **Managed Secrets** (ASM/GSM).""",
        ...
        if (
            "retrieve" in content_lower
            or "search" in content_lower
            or "read_url" in content_lower
            or "fetch" in content_lower
        ) and not any(
            kw in content_lower
            for kw in [
                "sanitize",
...
[bold green]RECOMMENDATION:[/bold green] Implement **Safety Critic** turns or Delimited XML blocks.""",
        ...
        if (
            (
                ...
                or "shutil" in content
                or "os.system" in content
                or "mcp" in content_lower
            )
            and "restricted" not in content_lower
            and "sandbox" not in content_lower
        ):
...
[bold green]RECOMMENDATION:[/bold green] Use **E2B Sandboxes**, **GVisor**, or **Docker Sandboxes** for tool execution.""",
        ...
        if (
            (
                "upsert" in content_lower
                or "index.add" in content_lower
                or "vector_store" in content_lower
            )
            and "verify" not in content_lower
            and "admin" not in content_lower
        ):
...
[bold green]RECOMMENDATION:[/bold green] Implement a **Vector Guardian** to validate document fidelity.""",
        ...
        if is_python:
            taint_findings = self._run_python_taint_scan(
        ...
        if re.search(
            r'(key|token|secret|password|auth|api_key)\s*=\s*["\'][A-Za-z0-9+/=_%.-]{16,}["\']',
            content,
            re.I,
        ):
    ...
    def _run_python_taint_scan(
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Assign):
                ...
                try:
                    call_str = (
                        ast.unparse(node.value).lower()
                        if hasattr(ast, "unparse")
                        else ""
                    )
                    if any(
                        s in call_str
                        for s in ["input(", "get_json", "params.get", "read()"]
                    ):
                    ...
                    for arg in node.args:
                        ...
                        for name in names:
                            if name in user_input_vars:
                                ...
                                recent_lines = content.splitlines()[
                                    context_start : node.lineno
                                ]
                                if not any(
                                    "sanitize" in line.lower()
                                    or "validate" in line.lower()
                                    for line in recent_lines
                                ):
                                    title = "cockpit Taint Flow: Unsanitized Data to Sink"
                                    if not self._is_ignored(
                                        node.lineno, content, title
                                    ):
                                        findings.append(
                                            AuditFinding(
                                                category="🛡️ SecOps",
