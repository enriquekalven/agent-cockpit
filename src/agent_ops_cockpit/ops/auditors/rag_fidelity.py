...
class RAGFidelityAuditor(BaseAuditor):
    ...
    def audit(
        ...
        is_rag_code = any(
            indicator in content.lower() for indicator in db_indicators
        )
        ...
        has_citations = any(
            kw in content.lower()
            for kw in ["source", "citation", "evidence", "grounding"]
        )
        ...
        if (
            "system_instructions" not in content
            and "<context>" not in content
            and ("prompt" in content.lower() or "template" in content.lower())
        ):
        ...
        if (
            "bigquery" in content.lower()
            or "snowflake" in content.lower()
            or "redshift" in content.lower()
        ):
            if "limit " not in content.lower() and (
                "select *" in content.lower() or "select " in content.lower()
            ):
        ...
        if (
            ...
            or "spanner" in content.lower()
            or "cloudsql" in content.lower()
        ) and "transaction" not in content.lower():
            if any(
                kw in content.lower()
                for kw in ["update", "set", "write", "commit", "delete"]
            ):
