...
class FinOpsAuditor(BaseAuditor):
    ...
    def simulate_opex_impact(
        self, current_tco: float, findings: List[AuditFinding]
    ) -> Dict:
        """
    ...
    def audit(
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.For, ast.While)):
                ...
                if any(
                    kw in loop_code
                    for kw in ["invoke", "generate", "predict", "query", "llm("]
                ):
                    ...
                    model_name = (
                        model_match.group(1) if model_match else "generic"
                    )
                    base_price = self.MODEL_PRICES.get(model_name, 1.25)
                    projected_tco = (
                        base_price * 10 * 2
                    )  # 10 iterations * multiplier
...
[bold green]RECOMMENDATION:[/bold green] Implement **Vertex AI Context Caching** or **cockpit Sliding Memory**.""",
        ...
        if any(re.search(m, content_lower) for m in high_tier_models):
            ...
            if any(
                kw in content_lower
                for kw in ["regex", "parse", "clean", "format"]
            ):
                findings.append(
                    AuditFinding(
                        category="💰 FinOps",
...
[bold green]RECOMMENDATION:[/bold green] Pivot to **Exponential Backoff** with jitter via `tenacity`.""",
        ...
        if "retrieve" in content_lower and (
            "top_k=100" in content_lower or "limit=100" in content_lower
        ):
            findings.append(
                AuditFinding(
                    category="💰 FinOps",
