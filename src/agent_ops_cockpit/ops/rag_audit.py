...
console = Console()
app = typer.Typer(
    help="RAG Truth-Sayer SME: Audits RAG pipelines for grounding and fidelity."
)
...
def perform_audit(path: str):
    ...
    console.print(
        Panel.fit(
            "🧗 [bold blue]RAG TRUTH-SAYER: FIDELITY AUDIT[/bold blue]",
            border_style="blue",
        )
    )
    ...
    for file_path in discovery.walk(path):
        ...
        if (
            discovery.is_library_file(file_path)
            or "auditors" in file_path
            or "frameworks.py" in file_path
        ):
            continue
    ...
    if not all_findings:
        console.print(
            "✅ [green]No RAG-specific risks detected or no RAG pattern found.[/green]"
        )
        return

    table = Table(
        title="RAG Fidelity Findings",
        show_header=True,
    ...
    for f in all_findings:
        table.add_row(
            f.title,
            f"{os.path.basename(f.file_path)}:{f.line_number or ''}",
            f.roi,
        )
