...
class BM25Engine:
    ...
    def score_field(
        self, query_tokens: List[str], field_text: str, weight: float
    ) -> float:
    ...
    def search(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        for _, doc in enumerate(self.corpus):
            total_score = 0.0
            total_score += self.score_field(
                query_tokens, doc.get("name", ""), self.field_weights["name"]
            )
            total_score += self.score_field(
                query_tokens,
                doc.get("description", ""),
                self.field_weights["description"],
            )
            total_score += self.score_field(
                query_tokens,
                " ".join(doc.get("tags", [])),
                self.field_weights["tags"],
            )
            total_score += self.score_field(
                query_tokens,
                doc.get("content", ""),
                self.field_weights["content"],
            )
