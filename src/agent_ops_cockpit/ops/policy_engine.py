...
class GuardrailPolicyEngine:
    ...
    def validate_input(self, prompt: str):
        ...
        if len(prompt) > max_len:
            raise PolicyViolation(
                "SECURITY", f"Prompt exceeds maximum allowed length ({max_len} chars)."
            )
    ...
    def check_tool_permission(self, tool_name: str) -> bool:
        ...
        require_hitl = self.policy.get("compliance", {}).get(
    ...
    def get_audit_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "forbidden_topics_count": len(
                self.policy.get("security", {}).get("forbidden_topics", [])
            ),
            "hitl_tools": self.policy.get("compliance", {}).get(
                "require_hitl_for_tools", []
            ),
            "token_threshold": self.policy.get("cost_control", {}).get(
                "max_tokens_per_turn"
            ),
        }
