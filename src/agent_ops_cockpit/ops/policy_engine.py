...
class GuardrailPolicyEngine:
    ...
    def __init__(self, policy_path: str = None):
        if not policy_path:
            ...
            yaml_path = os.path.join(
                os.path.dirname(__file__), "governance.yaml"
            )
    ...
    def _load_policy(self) -> Dict[str, Any]:
        ...
        with open(self.policy_path, "r") as f:
            if self.policy_path.endswith(".yaml") or self.policy_path.endswith(
                ".yml"
            ):
                data = yaml.safe_load(f)
                return (
                    data.get("policies", []) if isinstance(data, dict) else []
                )
            else:
                return json.load(f)
    ...
    def validate_input(self, prompt: str):
        ...
        if not topics and isinstance(self.policies, dict):
            topics = self.policies.get("security", {}).get(
                "forbidden_topics", []
            )
        ...
        for topic in topics:
            if re.search("\\b" + re.escape(topic) + "\\b", prompt.lower()):
                raise PolicyViolation(
                    "GOVERNANCE", f"Input contains forbidden topic: '{topic}'."
                )
        ...
        if len(prompt) > max_len:
            raise PolicyViolation(
                "SECURITY",
                f"Prompt exceeds maximum allowed length ({max_len} chars).",
            )
    ...
    def check_tool_permission(self, tool_name: str) -> bool:
        ...
        if not require_hitl and isinstance(self.policies, dict):
            require_hitl = self.policies.get("security", {}).get(
                "hitl_tools", []
            )

        if tool_name in require_hitl:
            console.print(
                f"⚠️ [bold yellow]HITL REQUIRED:[/bold yellow] Tool '{tool_name}' requires manual approval."
            )
            return False
        return True

    def enforce_cost_limits(
        self, estimated_tokens: int, accumulated_cost: float = 0.0
    ):
        ...
        if max_tokens is None and isinstance(self.policies, dict):
            max_tokens = self.policies.get("cost_control", {}).get(
                "max_tokens_per_turn"
            )
        ...
        if accumulated_cost >= max_budget:
            raise PolicyViolation(
                "FINOPS",
                f"Session budget exceeded (${accumulated_cost} >= ${max_budget}).",
            )
    ...
    def validate_codebase_rules(self, target_dir: str = "."):
        ...
        for file_path in discovery.walk(target_dir):
            ...
            for rule in rules:
                if (
                    "release execution" in rule.lower()
                    and "zero2hero" in file_path.lower()
                ):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Caught Expected Violation: {e.category} - {e.message}"
