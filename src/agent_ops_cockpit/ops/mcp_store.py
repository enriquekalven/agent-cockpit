...
app = typer.Typer(
    help="MCP Tool Store: Discover and integrate Model Context Protocol tools."
)
console = Console()
MCP_REGISTRY: List[Dict[str, Any]] = [
    {
        "name": "google-search",
        "type": "Tool",
        "desc": "Live web search via Google Search API",
        "mcp": "mcp-v1.0",
    },
    {
        "name": "pypi-inspector",
        "type": "Inspector",
        "desc": "Analyzes package health and security",
        "mcp": "mcp-v1.2",
    },
    {
        "name": "gemma-it",
        "type": "Bridge",
        "desc": "Low-latency Gemma 2 inference bridge",
        "mcp": "mcp-v1.1",
    },
    {
        "name": "vault-scf",
        "type": "Secure Tool",
        "desc": "HashiCorp Vault secret fetching",
        "mcp": "mcp-v1.3",
    },
]
...
def list():
    ...
    console.print(
        Panel.fit(
            "📦 [bold blue]MCP TOOL STORE: CENTRAL REGISTRY[/bold blue]",
            border_style="blue",
        )
    )
    ...
    console.print(table)
    console.print(
        "\n💡 [dim]Run 'agentops-cockpit mcp install <name>' to scaffold the integration.[/dim]"
    )
...
def install(name: str):
    ...
    console.print(
        f"🛠️ [bold cyan]Scaffolding integration for '{name}'...[/bold cyan]"
    )
    ...
    console.print(
        f"✅ [green]Successfully integrated '{name}' in mcp/{name.replace('-', '_')}.py[/green]"
    )
...
def generate_mcp_blueprint(path: str = typer.Option(".", "--path", "-p")):
    ...
    console.print(
        "🔧 [bold blue]Generating MCP Modernization Blueprint...[/bold blue]"
    )

    discovery = DiscoveryEngine(path)
    brain = discovery.find_agent_brain()

    mcp_code = f"""# 🛰️ Model Context Protocol (MCP) Server Wrapper
...
"""
    ...
    console.print(
        f"✨ [bold green]MCP Wrapper generated:[/bold green] {output_path}"
    )
    console.print(
        "[dim]Next step: Run 'pip install mcp' and migrate your logic into call_tool().[/dim]"
    )
