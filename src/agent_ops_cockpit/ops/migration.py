...
class MigrationEngine:
    ...
    def hydrate_agent(self, agent_path: str, target_cloud: str = "google"):
        ...
        cloud_header = header_map.get(
            target_cloud, "# v2.0.7 Multi-Cloud Optimized Agent\n"
        )

        # Remove any existing Cockpit Alignment header (any version)
        remediator.content = re.sub(
            r"^# v\d+\.\d+\.\d+(?:\.\d+)? Cockpit Alignment: .+\n",
            "",
            remediator.content,
            flags=re.MULTILINE,
        )
        ...
        if target_cloud == "google":
            for p, imp in providers.items():
                if p != "google" and imp in remediator.content:
                    remediator.content = remediator.content.replace(
                        imp,
            ...
            for p, imp in providers.items():
                if (
                    p not in ["azure", "microsoft"]
                    and imp in remediator.content
                ):
                    remediator.content = remediator.content.replace(
                        imp,
    ...
    def generate_deployment_assets(
        self, agent_path: str, target_cloud: str = "google"
    ):
...
"""
        ...
        if target_cloud == "google":
            ...
            content = (
                docker_base
                + 'CMD ["python", "agent.py"] # Managed by AWS App Runner / Lambda'
            )
            ...
            content = (
                docker_base
                + 'CMD ["gunicorn", "-w", "4", "agent:app"] # Optimized for Azure'
            )
    ...
    def auto_register_to_gemini(
        self,
        service_name: str,
        region: str = "us-central1",
        a2a_proxy: bool = False,
    ):
        """
        ...
        try:
            ...
            registration = {
                ...
                "api_spec": f"{service_url}/openapi.json"
                if service_url
                else "A2A_PROXY_PENDING",
                "auth": {
                    "type": "OIDC_ID_TOKEN" if not a2a_proxy else "A2A_HMAC"
                },
                "registered_at": datetime.now().isoformat(),
                "provider": "Agent Engine (A2A)"
                if a2a_proxy
                else "Gemini Enterprise",
                "interface": "A2UI v1.0",
            }

            reg_path = os.path.join(
                self.root_path, ".cockpit", "gemini_enterprise_registry.json"
            )
            os.makedirs(os.path.dirname(reg_path), exist_ok=True)

            if a2a_proxy:
                print(
                    f"🌉 Enabling A2A Bridge for Cross-Cloud agent: {service_name}..."
                )
            else:
                print(
                    f"📡 Pushing Tool Registration to Vertex AI Agent Engine: {registration['id']}..."
                )
            ...
            if a2a_proxy:
                ...
                return "A2A_FALLBACK"
            print(
                f"⚠️ Registration deferred: Service {service_name} not yet deployed."
            )
            return None
...
"""
    ...
    def run_migration_loop(self, target_cloud: str = "google"):
        ...
        for c in candidates:
            service_name = (
                c["name"].lower().replace(".py", "").replace("_", "-")
            )
            print(
                f"🚀 [Multi-Cloud] Migrating {c['name']} to {target_cloud.upper()}..."
            )
