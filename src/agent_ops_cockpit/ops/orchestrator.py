...
def parse_action_line(line: str) -> "AuditFinding":
    ...
    legacy_str = (
        action_str.split("| IPC_PAYLOAD:")[0]
        if "| IPC_PAYLOAD:" in action_str
        else action_str
    )
...
class CockpitOrchestrator:
    ...
    def __init__(self):
        ...
        self.report_path = os.path.join(
            self.output_root, "cockpit_final_report.md"
        )
        self.html_report_path = os.path.join(
            self.output_root, "cockpit_report.html"
        )
        ...
        for f_path in sorted(discovery.walk(path)):
            ...
            try:
                ...
                if f_path.endswith(
                    ...
                    with open(
                        f_path, "r", encoding="utf-8", errors="ignore"
                    ) as f:
                        content = f.read()
    ...
    def run_command(
        ...
        if sim or getattr(self, "sim", False):
            ...
            progress.update(
                task_id, description=f"[green]✅ {name} (SIM)", completed=100
            )
            return (name, True)
        ...
        isolated_venv = os.path.islink(
            os.path.join(os.getcwd(), ".cockpit_venv")
        ) or os.path.exists(os.path.join(os.getcwd(), ".cockpit_venv"))
        ...
        env["PYTHONPATH"] = (
            f"{config.get_python_path()}{os.pathsep}{os.pathsep.join(agent_paths)}{os.pathsep}{env.get('PYTHONPATH', '')}"
        )
        ...
        try:
            ...
            try:
                ...
                progress.update(
                    task_id,
                    description=f"[red]⌛ {name} Timeout",
                    completed=100,
                )
                return (name, False)

            if process.returncode != 0 and (
                "401" in stderr or "Unauthorized" in stderr
            ):
                console.print(
                    f"⚠️ [yellow]Registry Auth Failure detected in {name}. Retrying with Public PyPI Failover...[/yellow]"
                )
                env["UV_INDEX_URL"] = config.PUBLIC_PYPI_URL
                env["PIP_INDEX_URL"] = config.PUBLIC_PYPI_URL
                process = subprocess.Popen(
            ...
            if success:
                progress.update(
                    task_id, description=f"[green]✅ {name}", completed=100
                )
            else:
                progress.update(
            ...
            progress.update(
                task_id, description=f"[red]💥 {name} Error", completed=100
            )
            return (name, False)
    ...
    def generate_executive_summary(
        self, actions: List[str], as_html: bool = False
    ) -> List[str]:
        """v2.0.7 Executive Personality: Professional, High-Stakes Audit Persona."""
        if not actions:
            summary = [
                "The system audit indicates that all core pillars are currently aligned with the Governance Framework Framework. No immediate architectural drift or security regressions were detected."
            ]
            return (
                summary if not as_html else "<p>" + " ".join(summary) + "</p>"
            )
        ...
        p1_issues = sum(
            1
            for a in actions
            if any(x in a.lower() for x in ["secret", "security", "leak"])
        )
        p2_issues = sum(
            1
            for a in actions
            if any(x in a.lower() for x in ["reliability", "unit test"])
        )

        summary = []
        if p1_issues > 0:
            summary.append(
                f"CRITICAL: The audit has identified {p1_issues} high-impact security or compliance vulnerabilities that require immediate attention prior to production deployment."
            )
        elif p2_issues > 0:
            summary.append(
                f"WARNING: The system exhibits significant reliability drift ({p2_issues} P2 findings). While functioning, the agent is at risk of runtime instability in edge-case scenarios."
            )
        else:
            summary.append(
        ...
        summary.append(
            f"Total findings: {len(actions)}. Recommended path: Execute Phase 1 hardening and re-verify."
        )
    ...
    def print_terminal_v13_summary(self, developer_actions):
        ...
        def priority_key(action):
            p = action.lower()
            if any(
                (x in p for x in ["leak", "secret", "credential", "security"])
            ):
                return 0
            if any(
                (
        ...
        if getattr(self, "plain", False):
            console.print(
                f"--- SME Executive Summary ---\n{summary_text}\n-----------------------------"
            )
        else:
            console.print(
                Panel(
                    summary_text,
                    title="👔 Distinguished Fellow Executive Summary",
                    border_style="blue",
                )
            )
                ...
                for action in groups[p_val]:
                    ...
                    if len(parts) >= 3 and parts[1] not in seen:
                        findings_table.add_row(
                            f"P{p_val + 1}", cat_name, parts[1], parts[2]
                        )
                        seen.add(parts[1])
                    if len(seen) >= 3:
                        break
    ...
    def generate_report(self):
        ...
        status_icon = (
            "✅" if all(r["success"] for r in self.results.values()) else "❌"
        )
        ...
        if os.path.exists(self.lake_path):
            try:
                with open(self.lake_path, "r") as f:
                    ...
                    historical = lake_data.get(target_abs, {}).get(
                        "summary", {}
                    )
        ...
        report.append(
            f"| **Overall Health Score** | {health_score:.1f}% | {trend_icon} {delta:+.1f}% |"
        )
        report.append(
            f"| **Governance Gates** | {sum(1 for r in self.results.values() if r['success'])} / {len(self.results)} | - |"
        )
        report.append(
            f"| **Critical Anomalies** | {sum(1 for r in self.results.values() if not r['success'] and 'Security' in r.get('output', ''))} | - |"
        )
        ...
        configs_path = os.path.join(
            getattr(self, "target_path", "."), "cockpit.yaml"
        )
        ...
        if health_score < threshold:
            ...
            report.append(
                f"> The current audit health score ({health_score:.1f}%) is below your configured threshold ({threshold}%)."
            )
            report.append(
                "> This codebase contains architectural or security vulnerabilities that require immediate attention."
            )
        ...
        for name, data in self.results.items():
            is_pilot_error = "Traceback" in str(
                data.get("output", "")
            ) and "agent_ops_cockpit" in str(data.get("output", ""))
            status = (
                "✅ APPROVED"
                if data["success"]
                else ("🚨 PILOT ERROR" if is_pilot_error else "❌ REJECTED")
            )
            ...
            prio = (
                ...
                if any(
                    x in name.lower()
                    for x in ["secret", "security", "policy", "red"]
                )
                else "P2"
            )
            report.append(
                f"| {pillar} | {name} | {status} | {prio} | [View Specs]({doc_link}) |"
            )

        if developer_actions:
            report.append("\n## 🏗️ Tactical Implementation Plan")
            report.append(
                "Follow this prioritized roadmap to reach production-readiness."
            )

            def priority_key(action):
                p = action.lower()
                if any(x in p for x in ["leak", "secret", "security"]):
                    return 0
                if any(x in p for x in ["reliability", "unit test"]):
                    return 1
                return 2
        ...
        try:
            ...
            console.print(
                "📄 [bold green]Technical Design Document & Codebase Bundle updated.[/bold green]"
            )
        except Exception as e:
            console.print(
                f"⚠️ [yellow]Documentation generation failed:[/yellow] {e}"
            )
        ...
        console.print(
            f"📄 [bold blue]Printable HTML Report available at {self.html_report_path}[/bold blue]"
        )
    ...
    def apply_targeted_fix(self, issue_id: str, target_path: str = "."):
        ...
        if not os.path.exists(lake_path):
            console.print(
                "[red]❌ Error: No audit findings lake found. Run 'agent-ops report' first.[/red]"
            )
            return False
        ...
        if not agent_data:
            console.print(
                f"[red]❌ Error: No audit data found for path: {target_abs}[/red]"
            )
            return False
        ...
        for module_name, data in results.items():
            ...
            for line in output.split("\n"):
                if "ACTION:" in line:
                    finding = parse_action_line(line)
                    if finding.title:
                        current_id = hashlib.md5(
                            f"{module_name}:{finding.title}".encode()
                        ).hexdigest()[:8]
                        if (
                            current_id == issue_id
                            or issue_id
                            in finding.title.lower().replace(" ", "_")
                        ):
                            console.print(
                                f"🔧 [bold green]Targeted Fix:[/bold green] Applying remediation for '{finding.title}' in {finding.file_path}"
                            )
                            remediator = CodeRemediator(finding.file_path)
                            if (
                                "resiliency" in finding.title.lower()
                                or "retry" in finding.title.lower()
                            ):
                                remediator.apply_resiliency(finding)
                                remediator.save()
                                found = True
                            elif (
                                "zombie" in finding.title.lower()
                                or "timeout" in finding.title.lower()
                            ):
                            ...
                            if found:
                                console.print(
                                    f"✅ [bold green]Fixed {finding.title} successfully.[/bold green]"
                                )
                                return True
        console.print(
            f"[yellow]⚠️ Could not find a matching automated fix for issue ID '{issue_id}'.[/yellow]"
        )
        return False
    ...
    def get_exit_code(self):
        ...
        arch_fail = not self.results.get("Architecture Review", {}).get(
            "success", True
        )
        policy_fail = not self.results.get("Policy Enforcement", {}).get(
            "success", True
        )
        ...
        rel_fail = not self.results.get("Reliability (Quick)", {}).get(
            "success", True
        )
        if rel_fail:
            return 3
        ...
        red_fail = not (
            self.results.get("Red Team (Fast)", {}).get("success", True)
            and self.results.get("Red Team Security (Full)", {}).get(
                "success", True
            )
        )
        if red_fail:
        ...
        cost_fail = not self.results.get("Token Optimization", {}).get(
            "success", True
        )
        if cost_fail:
            return 5
    ...
    def save_to_evidence_lake(self, target_abs: str):
        ...
        try:
            ...
            console.print(
                f"[dim]Failed to create evidence lake symlink: {e}[/dim]"
            )
        lake_path = self.lake_path
        fleet_data = {}
        ...
        console.print(
            f"📜 [EVIDENCE LAKE] Partitioned log updated at {agent_dir}/latest.json"
        )
    ...
    def _generate_sarif_report(self, developer_actions):
        sarif = {
            ...
            "runs": [
                {
                    "tool": {"driver": {"name": "AgentOps Cockpit"}},
                    "results": [],
                }
            ],
        }
        for action in developer_actions:
            ...
            if len(parts) == 3:
                sarif["runs"][0]["results"].append(
                    {
                        "ruleId": parts[1].replace(" ", "_").lower(),
    ...
    def _generate_html_report(self, developer_actions, developer_sources):
        ...
        critical_count = sum(
            ...
            if not r["success"]
            and any(
                x in r.get("output", "").lower()
                for x in ["secret", "security", "leak"]
            )
        )
        agent_identity = os.path.basename(target_abs)
        ...
        all_findings_text = "\n".join(
            [r.get("output", "") for r in self.results.values()]
        )
        ...
        if any(
            x in all_findings_text.lower()
            for x in ["logging", "tracing", "telemetry"]
        ):
            mock_findings.append(
                AuditFinding(
                    category="💰 FinOps",
        ...
        opex_drivers = (
            ", ".join(impact["drivers"]) if impact["drivers"] else "Baseline"
        )
        opex_class = (
            "delta-pos"
            if opex_delta < 0
...
<body>
    <div class="app-container">
        ...
        for name, data in self.results.items():
            ...
            status_class = (
                "status-pass"
                if data["success"]
                else ("status-warning" if is_pilot_error else "status-fail")
            )
            status_text = (
                "APPROVED"
                if data["success"]
    ...
    def send_email_report(
        ...
        try:
            ...
            with open(
                self.report_path, "r", encoding="utf-8", errors="replace"
            ) as f:
                content = f.read()
    ...
    def load_latest_findings(self, path: str) -> List[AuditFinding]:
        ...
        if not os.path.exists(evidence_path):
            console.print(
                f"[dim]No evidence lake found at {evidence_path}[/dim]"
            )
            return []
        ...
        if not os.path.exists(agent_dir):
            ...
            if os.path.exists(latest_link):
                agent_dir = os.path.join(
                    evidence_path, os.readlink(latest_link)
                )
    ...
    def _discover_plugins(self, target_path: str) -> list:
        ...
        """
        plugins = []
        plugin_dirs = [os.path.join(target_path, ".cockpit", "auditors")]
            ...
            if os.path.exists(pdir):
                for f in os.listdir(pdir):
                    if (
                        ...
                        display_name = (
                            f.replace(".py", "").replace("_", " ").title()
                        )
                        plugins.append(
                            (
                                f"Plugin: {display_name}",
                                [sys.executable, plugin_path, target_path],
                            )
                        )
        return plugins
...
def run_audit(
    ...
    lake_agent_dir = os.path.join(
        orchestrator.output_root, "evidence_lake", agent_hash
    )
    os.makedirs(lake_agent_dir, exist_ok=True)

    # [v2.0.1] Navigability: Symlink the latest audit for human discovery
    latest_symlink = os.path.join(
        orchestrator.output_root, "evidence_lake", "latest_audit"
    )
    try:
    ...
    if config:
        console.print(
            f"⚙️ [dim]Loaded local Cockpit Config from {target_path}/cockpit.yaml[/dim]"
        )
        if config.get("exclude_checks"):
            console.print(
                f"🚫 [yellow]Excluded checks per local config: {config['exclude_checks']}[/yellow]"
            )
    agent_hash = hashlib.md5(target_path.encode()).hexdigest()
    partition_path = os.path.join(
        orchestrator.output_root, "evidence_lake", agent_hash, "latest.json"
    )
    source_data = None
    ...
    if source_data and (not apply_fixes):
        ...
        if source_data.get("hash") == current_hash:
            console.print(
                f"⚡ [bold green]SKIP:[/bold green] No changes detected in {target_path}. Reusing evidence lake artifacts."
            )
            ...
            generate_fleet_dashboard(
                {target_path: orchestrator.get_exit_code()}
            )
            return orchestrator.get_exit_code()
    subtitle = (
        "Essential checks for dev-velocity"
        if mode == "quick"
        else "Full benchmarks & stress-testing"
    )
    if cordon:
    ...
    with Progress(
        ...
        steps = [
            ...
            (
                "Token Optimization",
                [sys.executable, "-m", f"{base_mod}.optimizer", "audit"]
                + token_opt_args,
            ),
            (
                "Reliability (Quick)",
            ...
            (
                ...
                [
                    sys.executable,
                    "-m",
                    f"{base_mod}.eval.run_skills_eval",
                    "run",
                    target_path,
                ],
            ),
        ...
        if plugin_steps:
            console.print(
                f"🧩 [bold cyan]Plug-and-Play SDK:[/] Detected {len(plugin_steps)} domain-specific auditors."
            )
        ...
        if only:
            ...
            steps = [
                s for s in steps if any(o in s[0].lower() for o in only_lower)
            ]
            if not steps:
                console.print(
                    f"⚠️ [yellow]Incremental Filter Active:[/yellow] No auditors match '{', '.join(only)}'. Running full suite instead."
                )
                ...
                steps = [
                    ...
                    (
                        "MCP Logic Hub",
                        [sys.executable, "-m", f"{base_mod}.ops.mcp_store"],
                    ),
                ]
            else:
                console.print(
                    f"🧗 [bold blue]Incremental Audit Mode:[/bold blue] Only checking {len(steps)} requested personas."
                )
        if skip:
            steps = [
                s
                for s in steps
                if not any(
                    (o.lower() in s[0].lower().replace(" ", "_") for o in skip)
                )
            ]
        ...
        if excluded:
            steps = [
                s
                for s in steps
                if s[0] not in excluded
                and s[0].lower().replace(" ", "_") not in excluded
            ]
        ...
        with ThreadPoolExecutor(max_workers=len(steps)) as executor:
            future_map = {
                ...
                for name, cmd, task_id in task_ids
            }
            for future in as_completed(
                future_map
            ):  # Changed from future_to_audit to future_map
                name, success = future.result()
    orchestrator.title = title
    telemetry.track_event_sync(
        "audit_started", {"mode": mode, "path": target_path}
    )
    ...
    if apply_fixes:
        ...
        for item in developer_actions:
            ...
            if full_path not in remediators:
                ...
                if fix_files_count >= max_fix_files:
                    console.print(
                        f"⚠️ [bold yellow]Blast Radius Guard:[/] Skipping {os.path.basename(full_path)} to prevent PR Exhaustion (Limit: {max_fix_files} files)."
                    )
                    continue
            ...
            if interactive:
                ...
                if not Confirm.ask("Apply this remediation?"):
                    console.print(
                        "⏭️  [yellow]Skipping remediation per architect request.[/yellow]"
                    )
                    continue

            if any(
                x in title.lower() for x in ["resiliency", "retry", "backoff"]
            ):
                ...
                x in title.lower()
                for x in ["hallucination", "poka-yoke", "literal"]
            ):
                rem.apply_tool_hardening(finding)
            elif any(
                x in title.lower() for x in ["policy", "arithmetic", "logic"]
            ):
                # Autonomous Scaffolding
                from agent_ops_cockpit.ops.discovery import DiscoveryEngine

                lang = DiscoveryEngine(
                    os.path.dirname(full_path)
                ).detect_language()
                engine_path = rem.scaffold_policy_engine(
                    os.path.dirname(full_path), language=lang
                )
                console.print(
                    f"🏗️  [bold green]Policy Scaffolding Generated:[/bold green] {os.path.basename(engine_path)} created in {os.path.dirname(full_path)}"
                )
            elif any(
                x in title.lower()
                for x in ["logging", "tracing", "telemetry", "signal"]
            ):
                # Auto-Instrumentation
                rem.eject_telemetry_lib(os.path.dirname(full_path))
                console.print(
                    "📡 [bold green]Telemetry Ejected:[/bold green] lib/logger.ts and lib/trace.ts created."
                )
            elif any(
                x in title.lower()
                for x in ["lateral", "over-privilege", "privilege"]
            ):
                # Code-First Security Hardening
                rem.apply_privilege_gate(finding)
                console.print(
                    f"🛡️  [bold green]Privilege Gate Injected:[/bold green] Added tool_privilege_check to {os.path.basename(full_path)}"
                )
            elif any(
                x in title.lower() for x in ["passive", "retrieval", "rag"]
            ):
                rem.apply_passive_retrieval(finding)
                console.print(
                    f"🌉 [bold green]Managed RAG Refactor:[/bold green] Injected decider logic to {os.path.basename(full_path)}"
                )
            if any(
                x in title.lower()
                for x in [
                ...
                if not interactive and not os.environ.get("FORCE_EVOLUTION"):
                    console.print(
                        f"🛡️  [bold yellow]Cordon Gate:[/] Skipping high-impact structural split for [bold]{os.path.basename(full_path)}[/]. Run with --interactive to approve."
                    )
                    continue

                rem.apply_structural_split(finding)
                console.print(
                    f"🏗️  [bold green]Architectural Split Scaffold:[/bold green] Injected modular router recommendation to {os.path.basename(full_path)}"
                )
        ...
        for path, rem in remediators.items():
            if dry_run:
                ...
                if patch_path:
                    console.print(
                        f"🏜️  [yellow]DRY RUN: Patch generated at {patch_path}[/yellow]"
                    )
            else:
                rem.save_patch()
                console.print(
                    f"📦 [bold green]Autonomous Remediation Staged:[/bold green] Patch created for {os.path.basename(path)}"
                )
    ...
    for name, data in orchestrator.results.items():
        ...
        if len(surface_targets) > 3:
            ...
            for target in surface_targets:
                clustered_actions.append(
                    f"ACTION: {target}:1 | Missing GenUI Surface | Map this surface to the A2UI Protocol."
                )
        ...
        for title, files in generic_upgrades.items():
            if len(files) > 2:
                clustered_actions.append(
                    f"ACTION: Core Enhancements | {title} | Generic best practice upgrade recommended for {len(files)} files ({', '.join(files[:2])}...)"
                )
            else:
                for f in files:
                    # Restore actions filtered
                    clustered_actions.append(
                        f"ACTION: {f}:1 | {title} | Structural Enhancement."
                    )
    ...
    try:
        ...
        agg_script = os.path.join(
            os.getcwd(), "scripts", "aggregate_telemetry.py"
        )
        if os.path.exists(agg_script):
            subprocess.run(["python3", agg_script], capture_output=True)
            console.print(
                "📊 [bold cyan]Telemetry aggregated for Fleet Dashboard.[/bold cyan]"
            )
    except Exception:
        pass
    ...
    telemetry.track_event_sync(
        "audit_completed",
        {
            ...
            "exit_code": exit_code,
            "success_rate": sum(
                1 for r in orchestrator.results.values() if r["success"]
            )
    ...
    all_findings_text = "\n".join(
        [r.get("output", "") for r in orchestrator.results.values()]
    )
    finops_auditor = None
    ...
    impact = finops_auditor.simulate_opex_impact(
        100.0, real_findings
    )  # 100 as base%
    delta_str = (
        f"[green]-{abs(impact['delta']):.1f}%[/green]"
        if impact["delta"] < 0
        else f"[red]+{impact['delta']:.1f}%[/red]"
    )
    ...
    console.print(
        f"Strategic Drivers: [dim]{', '.join(impact['drivers']) if impact['drivers'] else 'Baseline'}[/dim]"
    )
    ...
    if apply_fixes:
        ...
        console.print(
            f"Recommended: Run [white]cockpit audit benchmark --path {target_path}[/white] to verify fixes against Golden Set."
        )
...
def run_autonomous_evolution(target_path: str = ".", branch: bool = True):
    ...
    console.print(
        Panel.fit(
            "🤖 [bold green]TRINITY AUTONOMOUS EVOLUTION[/bold green]\nTarget: [dim]{target_path}[/dim]",
            border_style="green",
        )
    )
            ...
            for line in data["output"].split("\n"):
                if "ACTION:" in line:
                    ...
                    if finding.title:
                        ...
                        full_path = os.path.abspath(
                            os.path.join(target_path, finding.file_path)
                        )
                        if not os.path.exists(full_path) or not os.path.isfile(
                            full_path
                        ):
                            continue
                        ...
                        if full_path not in remediators:
                            ...
                            if fix_files_count >= max_fix_files:
                                console.print(
                                    f"⚠️ [bold yellow]Blast Radius Guard:[/] Limiting evolution to {max_fix_files} files."
                                )
                                break
                            remediators[full_path] = CodeRemediator(full_path)
                            fix_files_count += 1
                        ...
                        if any(
                            ...
                            if not os.environ.get("FORCE_EVOLUTION"):
                                console.print(
                                    f"🛡️  [bold yellow]Cordon Gate:[/] Skipping high-impact evolution for [bold]{os.path.basename(full_path)}[/]. Set FORCE_EVOLUTION=1 to override."
                                )
                                continue
                        ...
                        if (
                            "resiliency" in title_lower
                            or "backoff" in title_lower
                        ):
                            rem.apply_resiliency(finding)
                            applied_count += 1
                        elif (
                            "timeout" in title_lower or "zombie" in title_lower
                        ):
                            rem.apply_timeouts(finding)
                            applied_count += 1
                            ...
                            "hardening" in title_lower
                            or "poka-yoke" in title_lower
                        ):
                            rem.apply_tool_hardening(finding)
                            applied_count += 1
                        elif (
                            "compaction" in title_lower
                            ...
                            or "gate" in title_lower
                        ):
                            rem.apply_mcp_gating(finding)
                            applied_count += 1
                        elif (
                            "privilege" in title_lower
                            or "shadow env" in title_lower
                            or "credential risk" in title_lower
                        ):
                            rem.apply_privilege_gate(finding)
                            applied_count += 1
                        elif (
                            "abstraction" in title_lower
                            or "monocultural" in title_lower
                        ):
                            ...
                            "split" in title_lower
                            or "monolith" in title_lower
                            or "structural" in title_lower
                        ):
                            rem.apply_structural_split(finding)
                            applied_count += 1
                        else:
    ...
    for _path, rem in remediators.items():
        if branch:
            ...
            if html_diff:
                console.print(
                    f"📊 [cyan]Generated Side-by-Side Analysis:[/] {html_diff}"
                )
    ...
    if branches:
        console.print(
            f"✅ [bold green]Evolution Complete![/bold green] Created {len(branches)} autonomous hardening branches."
        )
        for b in branches:
            console.print(f"👉 [cyan]Draft PR ready on branch: {b}[/cyan]")
    elif applied_count > 0:
        console.print(
            f"✅ [bold green]Evolution Complete![/bold green] Applied {applied_count} surgical fixes to code."
        )
    else:
        console.print(
            "✨ [bold blue]No critical gaps found. Agent is already evolved.[/bold blue]"
        )
...
def workspace_audit(
    ...
    if not agents:
        console.print(
            f"[yellow]No agent projects found in {root_path}[/yellow]"
        )
        return True

    console.print(
        f"🦾 [bold blue]Fleet Orchestrator:[/] Detected {len(agents)} Agent Silos. Launching concurrent audit..."
    )
    results = {}
    from rich.live import Live

    table = Table(
        title="🚢 Cockpit Fleet Audit Dashboard",
        show_header=True,
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    status = (
                        ...
                        if results[agent_path] == 0
                        else "[bold red]❌ FAIL[/bold red]"
                    )
                    table.add_row(
                        os.path.relpath(agent_path, root_path),
                        mode.upper(),
                        status,
                    )
                except Exception as e:
                    table.add_row(
                        os.path.relpath(agent_path, root_path),
                        mode.upper(),
                        f"[bold magenta]ERROR: {e}[/bold magenta]",
                    )
            ...
            for path, data in lake_data.items():
                ...
                for _check_name, check_data in mod_results.items():
                    ...
                    if check_data.get("success"):
                        passed_checks += 1
            prev_compliance = lake_data.get("global_summary", {}).get(
                "compliance", 0
            )
            current_compliance = (
    ...
    if os.path.exists(lake_path):
        try:
            ...
            has_security_gap = any(
                "secret" in str(d).lower() or "unauthorized" in str(d).lower()
                for d in lake_data.values()
            )
            has_interaction_void = any(
                "A2UI" in str(d)
                and not d.get("results", {})
                .get("Face Auditor", {})
                .get("success", True)
                for d in lake_data.values()
            )
            ...
            if has_security_gap and has_interaction_void:
                correlation_risks.append(
                    "🚨 [CORRELATION RISK]: Combined **Cockpitty Gap** and **Interaction Void** detected across fleet. Risk: Lethal commands could be triggered without user-facing confirmation screens."
                )

            # Detect: FinOps Leakage in multiple silos
            finops_fails = [
                p
                for p, d in lake_data.items()
                if not d.get("results", {})
                .get("Token Optimization", {})
                .get("success", True)
            ]
            if len(finops_fails) > len(agents) // 2:
                correlation_risks.append(
                    "💰 [CORRELATION RISK]: Systemic **Token Hemorrhage** detected. Over 50% of the fleet is using suboptimal model tiers for simple tasks."
                )
    ...
    if correlation_risks:
        console.print(
            Panel(
                "\n".join(correlation_risks),
                title="🧠 Master Architect: Cross-Silo Correlation",
                border_style="red",
            )
        )
