...
app = typer.Typer(
    help="Cockpit Project Auditor: Standards & Performance Essentials"
)
console = Console()
...
def run_scan(path: str, verbose: bool = False, context: dict = None):
                ...
                if context:
                    for f_item in res:
                        if context["cloud"] == "aws":
                            f_item.description = f_item.description.replace(
    ...
    if verbose:
        console.print(
            f"⏱️ [bold blue]Scan complete. Processed {file_count} files in {duration:.2f}s.[/bold blue]"
        )
    return all_findings
...
def apply_fixes(
    ...
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Skip saving patches"
    ),
):
    """
    ...
    console.print(
        Panel.fit(
            "🚀 [bold blue]COCKPIT: AUTOMATED FIX ENGINE[/bold blue]",
            border_style="blue",
        )
    )
    ...
    if not findings:
        console.print(
            "✅ [bold green]No remediable issues found. Architecture is hardened.[/bold green]"
        )
        return
    file_map = {}
    for f in findings:
    ...
    for file_path, file_findings in file_map.items():
        ...
        for f in file_findings:
            if "Resiliency" in f.title or "retry" in f.description.lower():
                ...
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Exponential Backoff Decorator[/green] ({f.title})"
                )
            elif "Zombie" in f.title or "Timeout" in f.title:
                remediator.apply_timeouts(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Async Timeout Guard[/green] ({f.title})"
                )
            elif "Caching" in f.title:
                remediator.apply_caching(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Performance Caching[/green] ({f.title})"
                )
            elif "Hardening" in f.title:
                remediator.apply_tool_hardening(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Schema Validation[/green] ({f.title})"
                )
            elif "Compaction" in f.title:
                remediator.apply_context_compaction(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Context Compaction Strategy[/green] ({f.title})"
                )
            elif "Reflection" in f.title:
                remediator.apply_cockpit_reflection(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Reasoning Reflection Loop[/green] ({f.title})"
                )
            elif "Over-Privilege" in f.title or "HITL Gate" in f.title:
                remediator.apply_mcp_gating(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]HITL MCP Gate[/green] ({f.title})"
                )
            elif "Monocultural" in f.title or "Provider Bias" in f.title:
                remediator.apply_cloud_abstraction(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Cloud Provider Abstraction[/green] ({f.title})"
                )
            elif "Legacy Intelligence" in f.title or "SDK Latency" in f.title:
                remediator.apply_manifest_drift_fix(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Frontier SDK Upgrade[/green] ({f.title})"
                )
            elif "Malformed MCP" in f.title or "Protocol Fragility" in f.title:
                remediator.apply_mcp_validation(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Protocol Manifest Fix[/green] ({f.title})"
                )
            elif "Passive Retrieval" in f.title or "Passive RAG" in f.title:
                remediator.apply_passive_retrieval(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Managed RAG Refactor[/green] ({f.title})"
                )
            elif "Structural Monolith" in f.title:
                remediator.apply_structural_split(f)
                applied_count += 1
                console.print(
                    f"   🛠️ Planned: [green]Architectural Split Scaffold[/green] ({f.title})"
                )
        if applied_count > 0:
            if dry_run:
                console.print(
                    f"🏜️ [yellow]DRY RUN: Skip saving patch for {file_path}[/yellow]\n"
                )
            else:
                patch_path = remediator.save_patch()
                console.print(
                    f"✨ [bold green]Patch generated:[/bold green] {patch_path}"
                )
                console.print(
                    f"👉 [bold yellow]Review with:[/bold yellow] [white]agent-ops workbench --path {path}[/white]\n"
                )
    console.print(
        "🏁 [bold]Plan-then-Execute cycle complete. Review patches to apply fixes.[/bold]"
    )
...
def propose_fixes(path: str = "."):
    ...
    console.print(
        Panel.fit(
            "🌿 [bold green]COCKPIT: AUTOMATED PR GENERATOR[/bold green]",
            border_style="green",
        )
    )
    findings = run_scan(path)
    if not findings:
        console.print(
            "✅ [bold green]Architecture is already optimal. No PR needed.[/bold green]"
        )
        return
    ...
    for file_path, file_findings in file_map.items():
        if not file_path.endswith(".py"):
            continue
        console.print(
            f"🔧 [bold cyan]Proposing fixes for {file_path}...[/bold cyan]"
        )
        remediator = CodeRemediator(file_path)
        applied = 0
        for f in file_findings:
    ...
    if modified_files:
        portal.commit_fixes(
            modified_files,
            f"fix: [Cockpit] resolve {len(findings)} architectural gaps",
        )
        console.print(
            f"\n✨ [bold green]All fixes committed to branch '{branch_name}'.[/bold green]"
        )
        console.print(
            f"👉 [bold cyan]Next Step:[/bold cyan] Run [white]git push origin {branch_name}[/white] to propose these changes to your team.\n"
        )
    else:
...
def audit(
    ...
    export: bool = typer.Option(
        False, "--export", help="Export reports in HTML format"
    ),
    ...
    console.print(
        Panel.fit(
            f"🏛️ [bold blue]{framework_name.upper()}: ENTERPRISE ARCHITECT REVIEW v1.8[/bold blue]",
            border_style="blue",
        )
    )
    console.print(
        f"Detected Stack: [bold green]{framework_name}[/bold green] | [bold cyan]Cloud Context: {context['cloud'].upper()}[/bold cyan] | [bold magenta]Framework: {context['framework'].upper()}[/bold magenta]\n"
    )
    with console.status(
        "[bold blue]Performing Multi-Modal Scan (AST + Behavior + Infra)..."
    ):
        ...
        for check_text, _rationale in section["checks"]:
            ...
            matching_finding = next(
                (
                    ...
                    if f.category[:2] == cat_prefix
                    and f.title.lower() in check_key
                ),
                None,
            )
            if matching_finding:
    ...
    if all_findings:
        ...
        console.print(
            Panel.fit(
                "📋 [bold yellow]CRITICAL FINDINGS & BUSINESS IMPACT (v1.3)[/bold yellow]",
                border_style="yellow",
            )
        )
        for f in all_findings:
            console.print(
                f"🚩 [bold red]{f.title}[/bold red] ({f.file_path}:{f.line_number or ''})"
            )
            console.print(f"   [dim]{f.description}[/dim]")
            console.print(
                f"   ⚖️ [bold green]Strategic ROI:[/bold green] {f.roi}"
            )
            console.print(
                escape(
                    f"ACTION: {f.file_path}:{f.line_number or 1} | {f.title} | {f.description}"
                )
            )
            impact_report.append(
                f"- **{f.title}**: {f.description} (Impact: {f.impact})"
            )
    latency_impact = (
        sum((1 for f in all_findings if "latency" in f.description.lower()))
        * 200
    )
    cost_risk = (
        "HIGH"
        if any(
            (
                f.category == "💰 FinOps" and "pro" in f.description.lower()
                for f in all_findings
            )
        )
        else "LOW"
        ...
        <body>
            <div class="container">
                ...
                <div style="margin-top: 60px; border-top: 1px solid #f1f5f9; padding-top: 20px; font-size: 0.8rem; color: #94a3b8; text-align: center;">
                    Generated by AgentOps Cockpit v2.0.7
                </div>
            </div>
        </body>
        </html>
