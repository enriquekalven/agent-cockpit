...
class FleetManager:
    ...
    def register_agent(
        self, name: str, path: str, cloud: str, endpoint: str, version: str
    ):
    ...
    def mothball_fleet(
        self, cloud: Optional[str] = None, agent_name: Optional[str] = None
    ) -> int:
        """
        ...
        mothballed_count = 0
        for agent in fleet:
            if (not cloud or agent["cloud"] == cloud) and (
                not agent_name or agent["name"] == agent_name
            ):
