...
class CockpitOrchestrator:
    ...
    async def run_pipeline(self, path: str, fleet: bool = False):
        ...
        console.print(
            Panel.fit(
                f"🚀 [bold blue]Cockpit Fleet Pipeline[/]\n[bold]Target:[/] {self.target_cloud}\n[bold]Mode:[/] {'Fleet Sync' if fleet else 'Single Agent'}",
                border_style="blue",
            )
        )
        ...
        if not pre_mod.run_preflight(path, target_cloud=self.target_cloud):
            console.print(
                "❌ [bold red]Cockpit Pipeline Aborted: Pre-flight verification failed.[/]"
            )
            return []
    ...
    def _discover_agents(self, path: str) -> List[str]:
        ...
        if "agent.py" in os.listdir(path) or "pyproject.toml" in os.listdir(
            path
        ):
    ...
    async def _process_single_agent(self, agent_path: str):
        ...
        try:
            ...
            console.print(
                f"  💧 Step 2: Hydrating for {self.target_cloud.upper()}..."
            )
            ...
            if not os.path.exists(agent_file):
                ...
                py_files = [
                    f for f in os.listdir(agent_path) if f.endswith(".py")
                ]
                if py_files:
            ...
            console.print(
                f"  🚀 Step 3: Production Deployment ({self.target_cloud})..."
            )
            ...
            console.print(
                f"  ✨ [green]Agent {agent_name} is LIVE![/] [dim]({deploy_url})[/]\n"
            )
            ...
            console.print(
                f"  ❌ [red]Pipeline Failed for {agent_name}:[/] {str(e)}"
            )
    ...
    async def _deploy_to_cloud(self, path: str):
        ...
        if self.target_cloud == "google":
            ...
            if os.path.exists(k8s_manifest):
                console.print(
                    f"    [dim]Detected GKE manifests, deploying {agent_name} to cluster...[/]"
                )
            ...
            if os.path.exists(os.path.join(path, "agent_engine_deploy.py")):
                console.print(
                    "    [dim]Detected Agent Engine script, deploying to Vertex AI...[/]"
                )
                return "AGENT_ENGINE_ID_PENDING"

            # Fallback to Cloud Run
            console.print(
                "    [dim]No Agent Engine script detected. Falling back to Google Cloud Run (Serverless)...[/]"
            )
    ...
    def mothball_fleet(self, cloud: str = None):
        ...
        console.print(
            f"📉 [bold green]FinOps Action:[/] Mothballed {count} agents in {cloud or 'all clouds'}. (Scale to zero optimized)"
        )
    ...
    def resume_fleet(self, cloud: str = None):
        ...
        console.print(
            f"📈 [bold green]FinOps Action:[/] Resumed {count} agents in {cloud or 'all clouds'}."
        )
    ...
    def render_fleet_map(self, path: str = "."):
        ...
        root_tree = Tree(
            f"🏛️  [bold blue]Cockpit Workspace:[/] [white]{os.path.basename(path)}[/]"
        )
        ...
        for agent_root in local_agents:
            ...
            for dp_agent in deployed_agents:
                ...
                if (
                    dp_agent.get("path") == rel_path
                    or dp_agent.get("path") == agent_root
                ):
                    deployment = dp_agent
                    break
            ...
            if deployment:
                status_color = (
                    ...
                    if deployment["status"] == "MOTHBALLED"
                    else "red"
                )
                status_icon = (
                    "🟢"
                ...
                agent_node = root_tree.add(
                    Text.from_markup(
                        f"🧠 [bold cyan]{agent_name}[/] [dim]({rel_path})[/]"
                    )
                )
                cloud_node = agent_node.add(
                    Text.from_markup(
                        f"🛰️  [bold magenta]{deployment['cloud'].upper()}[/] -> [dim]{deployment['endpoint']}[/]"
                    )
                )
                cloud_node.add(
                    Text.from_markup(
                        f"{status_icon} [bold {status_color}]Status: {deployment['status']}[/]"
                    )
                )
            else:
                agent_node = root_tree.add(
                    Text.from_markup(
                        f"📦 [bold white]{agent_name}[/] [dim](Local Only: {rel_path})[/]"
                    )
                )
        ...
        ghost_agents = [
            ...
            if not any(
                os.path.abspath(a["path"]) == os.path.abspath(la)
                or a["path"] == os.path.relpath(la, path)
                for la in local_agents
            )
        ]

        if ghost_agents:
            ghost_root = root_tree.add(
                "👻 [bold yellow]External Deployments (Ghost Agents)[/]"
            )
            for ga in ghost_agents:
                ga_node = ghost_root.add(
                    Text.from_markup(
                        f"🧠 [bold cyan]{ga['name']}[/] [dim](Remote Only)[/]"
                    )
                )
                status_color = (
                    "green"
                    if ga["status"] == "HEALTHY"
                    else "yellow"
                    if ga["status"] == "MOTHBALLED"
                    else "red"
                )
                ga_node.add(
                    Text.from_markup(
                        f"🛰️  {ga['cloud'].upper()} -> {ga['endpoint']} [[bold {status_color}]{ga['status']}[/]]"
                    )
                )
        ...
        console.print(
            Panel(
                root_tree,
                title="⚡ Cockpit Fleet Topology Map",
                border_style="blue",
                expand=False,
            )
        )
