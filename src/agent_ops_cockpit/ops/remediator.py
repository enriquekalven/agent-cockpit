        ...
        if hasattr(original_node, "name") and not self.found:
            ...
            retry_decorator = cst.Decorator(
                decorator=cst.Call(
                    ...
                    args=[
                        cst.Arg(
                            keyword=cst.Name("wait"),
                            value=cst.Call(
                                func=cst.Name("wait_exponential"),
                                args=[
                                    cst.Arg(
                                        keyword=cst.Name("multiplier"),
                                        value=cst.Integer("1"),
                                    )
                                ],
                            ),
                        ...
                        cst.Arg(
                            keyword=cst.Name("stop"),
                            value=cst.Call(
                                func=cst.Name("stop_after_attempt"),
                                args=[cst.Arg(value=cst.Integer("3"))],
                            ),
                        ),
                    ],
                )
...
class TimeoutGuardTransformer(cst.CSTTransformer):
    """Layout-preserving LibCST Transformer to inject 'timeout=30' into HTTP & API calls."""

    def __init__(self, timeout_val: int = 30):
    ...
    def leave_Call(
        ...
        if is_target_call:
            # Check if timeout is already an argument
            has_timeout = any(
                isinstance(arg.keyword, cst.Name)
                and arg.keyword.value == "timeout"
            ...
            if not has_timeout:
                ...
                new_args = list(updated_node.args) + [
                    cst.Arg(
                        ...
                        value=cst.Integer(str(self.timeout_val)),
                    )
                ]
...
class ShadowModelRouterTransformer(cst.CSTTransformer):
    """Layout-preserving LibCST Transformer to route OpenAI/Anthropic models to Gemini."""
    ...
    def leave_Assign(
        ...
        if is_model_var and isinstance(original_node.value, cst.SimpleString):
            val_str = original_node.value.value.strip('"').strip("'")
            if any(
                x in val_str.lower()
                for x in ["gpt-4", "gpt-3.5", "claude", "deepseek"]
            ):
                self.modified = True
                new_value = cst.SimpleString(f'"{self.target_model}"')
...
class CodeRemediator:
        ...
        try:
            ...
            class ResiliencyTransformer(cst.CSTTransformer):
                ...
                def _apply_retry(self, original_node, updated_node):
                    ...
                    if pos and (
                        self.target_line <= 1
                        or pos.start.line <= self.target_line <= pos.end.line
                    ):
                        self.found = True
                        dummy = cst.parse_module(
            ...
            self._apply_cst_transformer(
                ResiliencyTransformer(finding.line_number, self.content)
            )
        except ImportError:
            pass
    ...
    def apply_model_routing(self, target_model: str = "gemini-2.5-flash"):
        """Surgically routes all OpenAI/Anthropic/DeepSeek shadow model assignments to Gemini using LibCST."""
        try:
            self._apply_cst_transformer(
                ShadowModelRouterTransformer(target_model)
            )
        except Exception as e:
            print(f"Shadow Model Routing Failed: {e}")
    ...
    def apply_caching(self, finding):
        ...
        try:
            ...
            class CachingTransformer(cst.CSTTransformer):
                ...
                def leave_Call(self, original_node, updated_node):
                    ...
                    if pos and (
                        ...
                        if isinstance(
                        ...
                        ) and original_node.func.value in [
                            "Agent",
                            "App",
                            "LlmAgent",
                        ]:
                            ...
                            val = cst.Call(
                                ...
                                args=[
                                    cst.Arg(
                                        keyword=cst.Name("min_tokens"),
                                        value=cst.Integer("2048"),
                                    ),
                                    cst.Arg(
                                        keyword=cst.Name("ttl_seconds"),
                                        value=cst.Integer("600"),
                                    ),
                                ],
                            )
                            new_kw = cst.Arg(
                            ...
                            if not any(
                                arg.keyword
                                and arg.keyword.value == "context_cache_config"
                                for arg in new_args
                            ):
                                new_args.append(new_kw)
                            return updated_node.with_changes(
            ...
            self._apply_cst_transformer(
                CachingTransformer(finding.line_number, self.content)
            )
        except ImportError:
            pass
        ...
        try:
            ...
            class PokaYokeTransformer(cst.CSTTransformer):
                ...
                def __init__(self, target_line, module_code):
                    ...
                    self.found = False
                    self.added_import = (
                        "from typing import Literal" in module_code
                    )
                ...
                def leave_FunctionDef(self, original_node, updated_node):
                    ...
                    if pos and (
                        ...
                        dummy = cst.parse_statement(
                            "'''POKA-YOKE: Use Literal types for categorical parameters to prevent model hallucination.'''\n"
                        )
                        new_body_elements.insert(0, dummy)
                        return updated_node.with_changes(
                            body=updated_node.body.with_changes(
                                body=tuple(new_body_elements)
                            )
                        )
                    return updated_node

            self._apply_cst_transformer(
                PokaYokeTransformer(finding.line_number, self.content)
            )
        except ImportError:
            pass
    ...
    def _apply_decorator_cst(
        self, finding, decorator_code, import_code, import_check_str
    ):
        try:
            import libcst as cst
            from libcst.metadata import PositionProvider
            ...
            self._apply_cst_transformer(
                GenericDecoratorTransformer(finding.line_number, self.content)
            )
        except ImportError:
            pass
    ...
    def apply_manifest_drift_fix(self, finding):
        ...
        try:
            ...
            new_content = re.sub(
                r"gpt-3\.5(-turbo)?", "gpt-4o-mini", self.content, flags=re.I
            )
            new_content = re.sub(
                r"gemini-1\.0(-\w+)?",
                "gemini-2.0-flash",
            ...
            new_content = re.sub(
                r"claude-2", "claude-3-5-haiku", new_content, flags=re.I
            )
    ...
    def apply_passive_retrieval(self, finding):
        ...
        try:
                    ...
                    if not self.added_already:
                        ...
                        for i, stmt in enumerate(new_body):
                            if isinstance(
                                ...
                                isinstance(n, (cst.Import, cst.ImportFrom))
                                for n in stmt.body
                            ):
                                idx = i + 1
                ...
                def leave_Call(self, original_node, updated_node):
                    ...
                    if pos and (
                        ...
                        if (
                            ...
                            and original_node.func.value == "retrieve"
                        ):
                            self.found = True
                            dummy = cst.parse_expression(
                                "decider_should_rag(query) and dummy_func()"
                            )
            ...
            self._apply_cst_transformer(
                PassiveRetrievalTransformer(finding.line_number, self.content)
            )
        except ImportError:
            pass
...
"""
    ...
    def _get_new_content(self):
        ...
        sorted_edits = sorted(
            self.edits, key=lambda x: (x["sl"], x["sc"]), reverse=True
        )
        ...
        for e in sorted_edits:
            try:
                ...
                new_content = (
                    new_content[:start_off] + e["rep"] + new_content[end_off:]
                )
            except Exception:
                continue
        return new_content
    ...
    def save(self):
        ...
        Console().print(
            f"DEBUG: Saving {self.file_path}. Content len: {len(new_content)}"
        )
        try:
            with open(
    ...
    def save_html_diff(self) -> str:
        ...
        diff_dir = os.path.join(
            os.getcwd(), ".cockpit", "evidence_lake", "diffs"
        )
        ...
        diff_path = os.path.join(
            diff_dir, f"{file_slug}_evolution_{timestamp}.html"
        )
    ...
    def save_to_branch(self) -> str:
        ...
        try:
            ...
            self.save()
            subprocess.run(
                ["git", "add", self.file_path], check=True, capture_output=True
            )
            subprocess.run(
                [
                    "git",
                    "commit",
                    "-m",
                    f"docs: autonomous hardening for {os.path.basename(self.file_path)}",
                    "--no-gpg-sign",
                ],
                check=True,
                capture_output=True,
    ...
    def scaffold_policy_engine(
        self, target_dir: str, language: str = "typescript"
    ):
        """
