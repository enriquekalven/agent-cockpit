...
class GovernanceSkill(abc.ABC):
    ...
    @classmethod
    def parse_skill_md(
        cls, content: str, filename: str = "unknown"
    ) -> SkillMetadata:
        """
        ...
        for line in lines:
            ...
            if lower.startswith("## rationalizations") or lower.startswith(
                "### rationalizations"
            ):
                current_section = "rationalizations"
            elif lower.startswith("## verification") or lower.startswith(
                "### verification"
            ):
                current_section = "verification"
            elif lower.startswith(
                "## promptfoo integration"
            ) or lower.startswith("### promptfoo integration"):
            ...
            if (
                current_section == "rationalizations"
                and "|" in line
                and "Rationalization" not in line
            ):
                parts = [p.strip() for p in line.split("|") if p.strip()]
                if len(parts) >= 2:
                    rationalizations.append(
                        {"excuse": parts[0], "rebuttal": parts[1]}
                    )
