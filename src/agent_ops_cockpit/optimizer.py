...
def analyze_code(
    content: str, file_path: str = "agent.py", versions: Dict[str, str] = None
    ...
    if (
        "google.cloud.aiplatform" in content_lower
        or "vertexai" in content_lower
    ):
        if v_ai == "Not Installed":
            issues.append(
                OptimizationIssue(
                    "vertex_install",
                    "Install Modern Vertex SDK",
                    "HIGH",
                    "90% cost savings",
            ...
            try:
                if pkg_version.parse(v_ai) < pkg_version.parse("1.70.0"):
                    issues.append(
                        OptimizationIssue(
                            "vertex_legacy_opt",
                            "Situational Performance (Legacy SDK)",
                            "MEDIUM",
                            "20% cost savings",
                ...
                if "trace" not in content_lower and "otel" not in content_lower:
                    issues.append(
                        OptimizationIssue(
                            "gcp_observability",
                            "Observability: Agent Starter Pack",
                            "HIGH",
                            "SRE: Deep Tracing",
                            "Google Cloud agents require structured tracing. Use Agent Starter Pack patterns for OpenTelemetry/Cloud Trace integration.",
                            "+ from agent_starter_pack import tracing # Use ASP for observability",
                            package="agent-starter-pack",
                        )
                    )
            except Exception:
                pass
    ...
    if "openai" in content_lower:
        if openai_v != "Not Installed" and pkg_version.parse(
            openai_v
        ) < pkg_version.parse("1.0.0"):
            issues.append(
                OptimizationIssue(
                    "openai_legacy",
                    "Found Legacy OpenAI SDK",
                    "HIGH",
    ...
    if (
        "anthropic" in content_lower or "claude" in content_lower
    ) and "orchestra" not in content_lower:
        issues.append(
            OptimizationIssue(
                "anthropic_orchestration",
                "Anthropic Orchestration Pattern",
                "HIGH",
                "30% reliability boost",
    ...
    if (
        "autogen" in content_lower or "microsoft" in content_lower
    ) and "workflow" not in content_lower:
        issues.append(
            OptimizationIssue(
                "ms_workflows",
                "Microsoft Agent Workflows",
                "MEDIUM",
                "40% consistency boost",
    ...
    if (
        "bedrock" in content_lower or "boto3" in content_lower
    ) and "actiongroup" not in content_lower:
        issues.append(
            OptimizationIssue(
                "aws_action_groups",
                "AWS Bedrock Action Groups",
                "HIGH",
                "50% tool reliability",
    ...
    if (
        "gke" in content_lower or "kubernetes" in content_lower
    ) and "identity" not in content_lower:
        issues.append(
            OptimizationIssue(
                "gke_identity",
                "GKE Workload Identity",
                "HIGH",
                "100% security baseline",
    ...
    if (
        ...
        or "axios" in content_lower
    ) and "fetch" not in content_lower:
        issues.append(
            OptimizationIssue(
                "node_native_fetch",
                "Native Fetch API",
    ...
    try:
        ...
        has_http_lib = any(
            x in imports for x in ["requests", "aiohttp", "httpx", "urllib"]
        )
    except Exception:
            ...
            try:
                if pkg_version.parse(lg_v) < pkg_version.parse("0.1.0"):
                    issues.append(
                        OptimizationIssue(
                            "langgraph_legacy",
                            "Situational Stability (Legacy LangGraph)",
                            "HIGH",
                            "Stability Boost",
                            f"Your are on {lg_v}. Older versions lack the hardened StateGraph compilation. Upgrade is recommended.",
                ...
                pass
        if (
            "persistence" not in content_lower
            and "checkpointer" not in content_lower
        ):
            issues.append(
                OptimizationIssue(
    ...
    if (
        (has_llm_sdk or has_http_lib)
        and not has_retry_decorator
        and "tenacity" not in imports
    ):
        issues.append(
            OptimizationIssue(
                "resiliency_retries",
    ...
    if (
        "session" not in content_lower
        and "conversation_id" not in content_lower
    ):
        issues.append(
            OptimizationIssue(
                "session_management",
    ...
    if (
        "cloudsql" in content_lower
        or "psycopg2" in content_lower
        or "sqlalchemy" in content_lower
    ):
    ...
    if "crewai" in content_lower or "crew(" in content_lower:
        if (
            "manager_agent" not in content_lower
            and "hierarchical" not in content_lower
        ):
            issues.append(
                OptimizationIssue(
                    "crewai_manager",
                    "Use Hierarchical Manager",
                    "MEDIUM",
                    "30% Coordination Boost",
    ...
    if "rag" in content_lower or "retriev" in content_lower:
        if "chunk" not in content_lower and "atomic" not in content_lower:
            issues.append(
                OptimizationIssue(
                    "atomic_rag",
                    "Implement Atomic RAG",
                    "HIGH",
                    "30% Token Savings",
                    "You appear to be using RAG but no 'chunking' or 'atomic retrieval' logic was detected. Sending full documents kills margins.",
                    "+ docs = vector_db.search(query, chunk_limit=5)",
                )
            )
    if (
        "model" in content_lower
        and "router" not in content_lower
        and ("is_simple" not in content_lower)
    ):
        issues.append(
            OptimizationIssue(
                "tiered_orchestration",
                "Implement Tiered Orchestration",
                "HIGH",
                "70% Cost Savings",
                "No model routing detected. Use a 'Router Agent' to decide if a query needs a Pro model or a Flash model.",
    ...
    if any(
        ...
        issues.append(
            OptimizationIssue(
                "prompt_compression",
                "Token Density: Redundant English",
                "MEDIUM",
                "15% Token Savings",
                "Identified 'filler' tokens in system instructions. Compressing 'You are a helpful assistant who is very good at coding' to 'Expert coder' reduces baseline cost.",
                "- You are a helpful assistant...\n+ Expert coder",
            )
        )
    if (
        "model" in content_lower
        and "retry" not in content_lower
        and ("tenacity" not in content_lower)
    ):
        issues.append(
            OptimizationIssue(
                "quota_management",
                "Quota Management: Missing Backoff",
                "HIGH",
                "Resiliency & ROI",
                "High-volume model calls detected without Exponential Backoff. Failed requests due to rate-limiting represent wasted compute and broken ROI.",
    ...
    if "tool" in content_lower or "pydantic" in content_lower:
        if "literal" not in content_lower:
            issues.append(
                OptimizationIssue(
                    "tool_hardening",
                    "Tool Schema Hardening (Poka-Yoke)",
                    "HIGH",
                    "Trajectory Stability",
                    "Your tool definitions lack strict type constraints. Using Literal types for categorical parameters prevents model hallucination and reduces invalid tool calls.",
                    "+ from typing import Literal\n+ def my_tool(category: Literal['search', 'calc', 'email']): ...",
                )
            )
    if (
        "history" in content_lower
        or "messages" in content_lower
        or "conversation" in content_lower
    ):
        if (
            "summary" not in content_lower
            and "trim" not in content_lower
            and ("compact" not in content_lower)
        ):
            issues.append(
                OptimizationIssue(
                    "context_compaction",
                    ...
                    'Long conversation history detected without a compaction or summarization strategy. This can lead to "Lost in the Middle" errors and increased token pressure.',
                    "+ def compact_history(messages): ...Summarize earlier turns...",
                )
            )
    if (
        "Agent(" in content or "LlmAgent(" in content
    ) and "context_cache_config" not in content:
        issues.append(
            OptimizationIssue(
                "adk_context_caching",
                "Enable ADK Context Caching",
                "HIGH",
                "90% cost savings",
                "ADK Agent detected without context caching. Enable it to reduce token costs.",
...
def estimate_savings(
    token_count: int, issues: List[OptimizationIssue]
) -> Dict[str, Any]:
    baseline_cost_per_m = 10.0
    monthly_requests = 10000
    current_cost = (
        token_count / 1000000 * baseline_cost_per_m * monthly_requests
    )
...
def audit(
    ...
    apply_fix: bool = typer.Option(
        False, "--apply", "--fix", help="Automatically apply recommended fixes"
    ),
    ...
    console.print(
        Panel.fit(
            "🔍 [bold blue]GCP AGENT OPS: OPTIMIZER AUDIT[/bold blue]",
            border_style="blue",
        )
    )
    ...
    if os.path.isdir(file_path):
        ...
        if not os.path.exists(file_path):
            console.print(
                f"❌ [red]Error: No python entry point found in {discovery.root_path}[/red]"
            )
            raise typer.Exit(1)
    ...
    console.print(
        f"📊 Token Metrics: ~[bold]{token_estimate:.0f}[/bold] prompt tokens detected."
    )
    ...
    if comp_reports:
        ...
        for report in comp_reports:
            if report["type"] == "INCOMPATIBLE":
                console.print(
                    f"❌ [bold red]Conflict Detected:[/bold red] {report['component']} + {report['conflict_with']}"
                )
                console.print(f"   [dim]{report['reason']}[/dim]")
            elif report["type"] == "SYNERGY":
                console.print(
                    f"✅ [bold green]Synergy Verified:[/bold green] {report['component']} is optimally paired."
                )
    if not issues:
        console.print(
    ...
    for opt in issues:
        ...
        if opt.evidence and "error" not in opt.evidence:
            ...
            if ev.get("upgrade_required"):
                console.print(
                    "🚨 [bold yellow]URGENT UPGRADE RECOMMENDED[/bold yellow]"
                )
                console.print(
                    f"   Current: {ev['installed_version']} | Required for optimization: >={ev['min_optimized_version']}"
                )
                ev_title = "[bold red]UPGRADE REQUIRED Evidence[/bold red]"
            ev_panel = Panel(
                f"🔗 [bold]Source[/bold]: {ev['source_url']}\n📅 [bold]Latest Release[/bold]: {ev['release_date'][:10]}\n📝 [bold]Note[/bold]: {ev['best_practice_context']}",
                title=ev_title,
            ...
            console.print(
                f"SOURCE: {opt.title} | {ev['source_url']} | {bp_context}"
            )
        ...
        console.print(
            escape(
                f"ACTION: {file_path} | Optimization: {opt.title} | {opt.description} (Est. {opt.savings})"
            )
        )
        do_apply = False
        ...
        if apply_fix:
            ...
            do_apply = typer.confirm(
                "\nDo you want to apply this code-level optimization?",
                default=True,
            )
        if do_apply:
