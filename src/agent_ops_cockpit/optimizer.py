...
def analyze_code(
    ...
    if "openai" in content_lower:
        if openai_v != "Not Installed" and version.parse(openai_v) < version.parse(
            "1.0.0"
        ):
            issues.append(
                OptimizationIssue(
...
def audit(
    ...
    if os.path.isdir(file_path):
        ...
        if not found:
            console.print(
                f"❌ [red]Error: No python entry point found in {file_path}[/red]"
            )
            raise typer.Exit(1)
    ...
    for opt in issues:
        ...
        console.print(
            f"ACTION: {file_path}:1 | Optimization: {opt.title} | {opt.description} (Est. {opt.savings})"
        )
