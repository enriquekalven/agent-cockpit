The **Cockpit** is the mission control for your AI agents. While the "Engine" handles reasoning, the Cockpit ensures that reasoning is safe, cost-effective, and auditable. In v2.0.17, the Cockpit has evolved from a dashboard into an **Autonomous Evolution Engine** with standardized security middleware.
...
*   **Access**: Securely managed via the **`uvx agentops-cockpit` CLI** or the integrated **Operational Guide**.
...
## 🛡️ Standardized Guardrails (v2.0.17)
...
*   **Autonomous Architect**: Runs `uvx agentops-cockpit audit arch` to generate ADRs and ROI analysis. [**Read Architect Guide**](TECHNICAL_ARCH_REVIEW.md).
...
*   **External Repos**: Use `uvx agentops-cockpit audit report --path <repo> TARGET=/path/to/remote-repo`. The Discovery Engine will automatically identify the agent's entry point and apply governance standards.

---
*Generated by the AgentOps Cockpit Orchestrator. Global Governance Division (v2.0.17).*
