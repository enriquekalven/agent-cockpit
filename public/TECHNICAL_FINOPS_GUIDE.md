...
| **Token Scan** | `uvx agentops-cockpit audit report` | Scans for model routing waste. | **FinOps SME Reasoning**. |
| **Certify Project** | `uvx agentops-cockpit certify` | Production ROI validation. | Cockpit Goal Alignment. |
| **Apply Optimization**| `uvx agentops-cockpit evolve`| Injects caching & routing logic. | **The Closer Engine**. |
...
## 🧗 Deep Dive: Reasoning Density (v2.0.19 Autonomous Core)
The primary KPI for the FinOps persona is **Reasoning Density (RD)**:
$$RD = \frac{QualityConsensusScore}{TokensUsed \times 10^{-3}}$$

*   **Optimization Loop**: `uvx agentops-cockpit audit report --mode deep` runs multiple iterations of your agent against a golden dataset.
...
*   **Compress Prompt**: Run `uvx agentops-cockpit evolve` to autonomously strip filler tokens from system instructions.
