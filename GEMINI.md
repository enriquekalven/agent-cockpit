...
1. Write/modify code in `my_super_agent/`
...
```bash
uv run python -m my_super_agent.app_utils.deploy --set-secrets "API_KEY=my-api-key,DB_PASS=db-password:2"
```
...
session_resp = requests.post(
    f"{SERVICE_URL}/apps/my_super_agent/users/{user_id}/sessions",
    headers=headers,
    json={"state": {}}
)
...
Add an `/invoke` endpoint to `my_super_agent/fast_api_app.py` for batch/event processing. The endpoint auto-detects the input format (BigQuery Remote Function, Pub/Sub, Eventarc, or direct HTTP).
...
gcloud pubsub subscriptions create my-sub --topic=my-topic \
    --push-endpoint=https://agent-cockpit.run.app/invoke
# Eventarc trigger
gcloud eventarc triggers create my-trigger \
    --destination-run-service=agent-cockpit \
